...
> ✅ **已於 v2.21 執行**：`tau_target` 由 0.8–3.0 收緊為 **1.0–2.0 Pa**。基線 1.574 Pa 通過，84 測試 0 失敗。帶寬減半，`K_tau` 被釘得更死。這是 V14 收緊（100–1500 → 300–1500）的同一種做法 —— **依原文收緊自己設寬的帶**。
>
> 值得一提的是，這兩次收緊都讓一個**已通過**的檢核變得更難通過，而不是相反。自訂的帶若從未被收過，多半是因為沒人回頭比對原文。
