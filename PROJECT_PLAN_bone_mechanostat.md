...
1. 皮質改用 **b = 0.5**（`(1−f)^0.5`），小樑維持圓頂形式（Martin 的區間本來就是小樑）；作法同 P5m，由 `trabecularParams` 覆寫。
2. 重擬並複驗 V1（骨轉換）、V2、**V11**、以及 P3。
3. 稿件 Limitations 目前寫「specific surface 是佔位函數形式、兩腔室共用一條曲線」—— 修完後要改寫，並加上「個體專一」這一點。

**未在本輪改動**：這會動到校正，且 V11 是直接受影響的標的。
