...
1. **先確認下游飽和**：在廢用情境下量 SOST 力學抑制項的飽和程度。若已飽和，則**任何經由感測的機制都補不上缺口**，包括力學驅動的凋亡（因為它也是經由 `n_ot` → 感測）。
2. 若確為飽和，缺口必須由**不經感測**的路徑供給。候選：凋亡骨細胞**直接**釋出 RANKL（繞過 SOST）、或 `lambda_xi` 在廢用時的表面分配偏移。
3. 補上之後才採用 `s2_Sv = 0.5` 並重擬。

**第 1 步是廉價的，且會一次決定第 2 步該往哪走 —— 下一輪從它開始。**
