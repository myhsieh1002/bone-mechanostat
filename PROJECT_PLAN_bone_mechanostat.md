...
**版本**：v2.11｜**日期**：2026-07-26｜**實作語言**：MATLAB (R2026a)｜**執行者**：Claude Code

> **v2.11 變更摘要（P5i：modeling drift，V6e 達標）**：**先試而後棄**：把吸收改為形成的倒數（補 mechanostat 的另一半）實測**方向相反** —— 通道劑量已飽和，骨膜表面相對增益反而較小，吸收被推向骨膜。已回退，診斷保留（C22.1）。**定案改走 Frost modeling drift 成對性**：骨膜沉積伴隨骨內膜吸收使皮質整體外移，`dr_e += chi_drift·modeling`，取 **`chi_drift = 1.0`（純平移，原理選擇非擬合，故 V6e 仍為 hold-out）**。**V6e 髓腔 −2.88% → +13.44%，V6 六項全數在容差內**（v2.10 為 5/6）；drift 掛在 modeling 上，繼承其應變閾值，故 V1/V2/V7/V8/V10/V14/V16 逐字不變，皮質壁厚由 3.100 平移至 3.196 mm。**⚠️ 代價：P1 第二句在 aBMD 上失守**（阻力訓練 +5.57% → +1.57%，低於 >4%）。掃描顯示 V6e 需 `chi ≳ 0.85`、P1(aBMD) 需 `chi ≲ 0.2`，**無交集**。內容有二：近半是 **DXA 尺寸假影**（同一對比以 BMC 計為 +3.09%，DXA 稀釋 49%），另一半是**負荷強度**（力矩 ×4.0、2900 με 時 BMC +4.50% ✅）。**故 P1 第二句在「BMC + 適當強度」成立、在「aBMD + 現行情境」不成立** —— 與 C18.2 同型，**P1 三個子句都對度量敏感，論文須逐句寫明**。三個互斥的處置選項列於 C22.5，**待主持人裁決**。81 測試全過。詳見附錄 C22。
>
> **v2.10 變更摘要（P5h：停藥反彈機制）**：C20.4 的診斷獲證實。新增**共用系統性狀態 `A_reb`** —— 抗體長期暴露下 SOST 轉錄的代償性上調，`dA/dt = (sost_reb·u_romo − A)/tau_reb`。抗體加的是**清除**、代償加的是**生產**，而**兩者時鐘不同**：停藥瞬間清除消失、升高的生產尚在 → 硬化蛋白過衝 → 吸收湧現 → BMD 回落。狀態數 16→**17**（雙站 29→**30**）。**隔離是結構性的**：`A_reb` 由 `u_romo` 驅動，無藥情境恆為 0，故 V1/V2/V7/V6/V14 **原理上不可能被擾動**（實測逐字驗證，V6 = 25.7490/10.6579/1.2766 完全相同）。**方法論核心：校正到新標的 V16（停藥後 CTX 過衝，臨床 1.2–1.4，已入 CSV）而非 V10** —— 若擬合 V10 就會摧毀本專案最有價值的盲測證據。結果：**V10 由 +1.683% → −12.416%，達標且維持 hold-out 身分**；V8 = 12.497% 仍在帶內；V16 = 1.305。(`delta_ab`, `sost_reb`) 為沿 V8 脊的相關對，**由 V16 打破簡併**（`delta_ab` 0.24875 → **1.65048**，`sost_reb` = 1.80）。**附帶：V9 強形式一併解決** —— `A_reb` 在治療期間就追上抗體（硬化蛋白第 1 月 0.50 → 第 9 月 1.03），**同一機制同時解釋自限性與停藥反彈**。已知簡化：藥物為階躍函數，故硬化蛋白尖峰幅度被誇大、CTX 峰在停藥後 3 週而非臨床的 3 個月（被校正的是幅度 V16，非峰形）。79 測試全過。詳見附錄 C21。
>
> **v2.9 變更摘要（P5g：`delta_ab` 重擬）**：修的不只是數字，是**目標函數裡的範疇錯誤** —— `evalTargets` 一直把脊椎標的 V8/V10 算在皮質模型上還放進 `chi2`，而 `delta_ab` 舊值 0.09184 正是被那個（C14 已證為礦化假影的）V8 釘住的。改以 `trabecularParams` 於**小樑腔室**評估 V8/V10 並另報 `V8cort`，再重擬 `delta_ab` → **0.24875（×2.709）**。`delta_ab` 僅在用藥時生效，故目標函數**可分離**，單參數重擬即為正解：**V1/V2/V7/V14/V6 全部逐字不變**（V6 盲測 25.749/10.658/1.277 完全相同），**V8 = 12.506% 首次真正達標**（帶 11–14；`test_calibration` 因此由「只斷言方向」升級為斷言帶）。皮質/小樑放大倍率 3.6× → **5.2×**。**同時更正 C19.4 的錯誤預測**：V10 對 `delta_ab` **非單調**，過零在 ×4（該處 V8 已 +18.45%），故**兩者無法由單一參數同時滿足**。**V10 的正確診斷是時間尺度問題**：拉到 6 年 aBMD **確實逆轉**（停藥後 2.85 年達峰，第 6 年跌回 12 個月水準以下），慢約 5 倍；`lambda_S`、`K_L` 掃遍文獻全域皆無效，因為硬化蛋白過衝本身只有 +7.2%（mechanostat 的後果而非參數）—— **缺的是抗體停藥的藥理反彈（P5h）**。另新增 `turnoverRate.m`（閉式周轉率，切斷 `evalTargets ↔ trabecularParams` 遞迴）。72 測試全過。詳見附錄 C20。
>
...
1. **P1 — 許可性 vs 指令性**（**v2.11 依 C22/C23 重述；子句 2 與 3 的度量已釘死**）。
   - **子句 1（鈣為許可性）**：在鈣攝取充足區間（800→1500 mg/day），鈣攝取量對 24 個月 **aBMD** 的邊際效益 **< 1%**。以 aBMD 陳述是刻意的：鈣作用於礦物質而非尺寸，兩種度量在此一致，且對照文獻（Tai 2015）本就以 aBMD 報告。→ 實測 **+0.470%** ✅
   - **子句 2（負荷為指令性）**：**適當**負重方案 —— 定義為峰值應變 **≈2900 με**（`resistanceVigorous` 情境，仍低於 3000 με 生理上限）—— 於同期間使**骨量 BMC** 增加 **> 4%**。→ 實測 **+4.589%** ✅
     - **為何改以 BMC 而非 aBMD**：本模型含 Frost modeling drift（P5i），負荷使皮質整體外移，而 $aBMD = A_g f_{bm}\rho/(2r_p)$ 除以投影寬度 —— **DXA 因此稀釋了 59% 的負荷反應**（同一對比 aBMD 僅 +1.865%）。此為 `densitometry` 刻意保留的尺寸假影，亦即臨床上 DXA 低估骨膜適應的已知現象。**以 aBMD 陳述會把一個度量假影誤讀為生物學結論。**
     - **為何「適當」必須釘死**：中等強度（`resistance`，2236 με）僅得 BMC +3.093%，不足 4%。原文的「適當負重方案」未定義強度，使該子句無法證偽。
   - **子句 3（交互作用）**：鈣不足會**截斷**負荷所能建立的骨量 —— 以**絕對增益**衡量（嚴重缺鈣 +3.938% vs 補充 +4.878%，缺鈣代價占增益的 19%）✅。⚠️ 但以**差異中的差異**衡量則符號**反轉**（−0.536 個百分點），因為缺鈣使久坐比較組掉得更快 —— 那是比較組的地板效應，不是協同。**論文必須採絕對框架並說明理由**（附錄 C18.2）。
   - **方法學結論（值得單獨發表）**：P1 的三個子句**都對度量敏感** —— 子句 2 隨 aBMD/BMC 翻轉、子句 3 隨絕對/差異框架翻轉。**同一個模型可以支持相反的結論，端看度量選擇。** 論文須逐句寫明度量與情境。詳見附錄 C23。
...
| **E2** | **因子設計：補鈣 × 負重** | {100, 400, 800, 1500 mg/day} × {久坐, 阻力訓練 2236 με, **適當負重 2949 με**}，24 個月 | **ΔBMC%（子句 2、3 的主要度量）＋ ΔaBMD%（子句 1，並顯示 DXA 稀釋）**；**驗證 P1**。⚠️ 度量選擇會翻轉子句 2 與 3 的結論 —— 見 C23 |
...
1. **主論文（方法／模型）**：*Biomechanics and Modeling in Mechanobiology*、*PLOS Computational Biology*、*Bone* 或 *Journal of Theoretical Biology*。
   ...
   **v2.11 補充 —— 度量必須逐句寫明**：P1 三個子句都對度量敏感（子句 2 隨 aBMD/BMC 翻轉、子句 3 隨絕對/差異框架翻轉）。論文須在陳述每個子句時同時給出度量與情境強度，並把「同一模型在不同度量下給出相反結論」本身當作一個方法學結果來報告 —— 這對讀者選擇臨床終點（DXA aBMD vs pQCT BMC）有直接意涵。詳見附錄 C23。
...
近線性，**標的 +11–14% 落在 ×2.5–3 之間**。

> **⚠️ v2.9 更正（P5g 實測）**：本節原寫「`delta_ab` 夠大到滿足 V8 時，V10 也自動翻為正確的負號 —— 兩個標的由同一個參數控制」。**這是誤讀上表，不成立。** V10 對 `delta_ab` **非單調**（先升後降），過零點在 ×4 附近，而該處 V8 已達 +18.45%，遠高於 11–14 帶。在 V8 達標的 ×2.4–3.0 區間，V10 仍為 **+1.7 ~ +1.4**。**V8 與 V10 無法由 `delta_ab` 單獨同時滿足。** 實際重擬結果與 V10 的真正診斷見附錄 C20。
...
## 附錄 C20：P5g —— `delta_ab` 重擬，**V8 首次成為真正的定量標的**；V10 的診斷改寫 (2026-07-26)

**修的不只是一個數字，是一個範疇錯誤：`evalTargets` 一直把脊椎標的 V8/V10 算在皮質模型上，還放進 `chi2` 目標函數。改到正確腔室後重擬 `delta_ab`，V8 = 12.51%（帶 11–14）首次真正達標。V10 仍未達 —— 但 C19 對它的預測是錯的，本附錄更正並給出正確診斷。**

### C20.1 真正的缺陷：目標函數裡的範疇錯誤

C14 早已判定 V8/V10 屬小樑/脊椎範疇，但 `evalTargets` 仍以皮質模型計算兩者，且 `r.resid.V8`、`r.chi2` 皆含之。亦即**校正目標函數內含一個算在錯誤腔室上的標的**，而 `delta_ab` 正是被它釘住的參數 —— 舊值 0.09184 是 v2.0 對著**已被 C14 證為礦化假影**的那次 V8 擬合出來的。

修正：`evalTargets` 以 `trabecularParams(p)` 建小樑腔室來評估 V8 與 V10，並**另行回報 `r.V8cort`**（因 `delta_ab` 為共用參數，不能只顧脊椎而讓皮質跑掉）。

**附帶重構**：新增 `src/analysis/turnoverRate.m`，把周轉率的閉式解抽出（`S_v_hat` 在各腔室自身基線恆為 1，故無須模擬）。這同時 (a) 讓 `trabecularParams` 以**一次除法**而非擬合迴圈求 `k_res`，(b) **切斷 `evalTargets ↔ trabecularParams` 的相互遞迴**（前者現在會呼叫後者）。

### C20.2 為何單參數重擬就是正解（而非重跑五參數校正）

`delta_ab` 只出現在 `osteocyteSignal` 的一處：`clr = delta_S + delta_ab * u_romo`。**只有用藥時生效。** 故：

- **V1 / V2 / V7 / V7slope / V6 / V14 在數學上完全獨立於 `delta_ab`** —— 實測逐字驗證（V6 盲測 BMC +25.749%、Tot.Ar +10.658%、vBMD +1.277%，與 v2.8 完全相同）。
- 另外四個自由參數目前使 V1/V2/V7/V7slope **全部落在帶內**，帶狀殘差為 0，已在最適點；把 V8 換腔室不會移動它們。

目標函數因此**可分離**：四參數對 V1/V2/V7，`delta_ab` 對 V8。以幾何二分（4 次迭代）對帶中點 12.5% 求解。

### C20.3 結果

| 標的 | v2.8 | **v2.9** | 帶 | |
|---|---|---|---|---|
| V1 周轉 | 7.192 | 7.192 | 5–10 | ✅ 逐字不變 |
| V2 廢用 | 1.153 | 1.153 | 1.0–1.5 | ✅ 逐字不變 |
| V7 鈣 | 1.140 | 1.140 | 0.7–1.8 | ✅ 逐字不變 |
| V7 斜率 | −0.238 | −0.238 | \|·\|<0.3 | ✅ 逐字不變 |
| **V8（小樑）** | 4.471 | **12.506** | 11–14 | ✅ **首次真正達標** |
| V8cort（皮質臂） | 1.247 | 2.406 | > 0 | ✅ 方向正確、量級仍屬皮質 |
| **V10（小樑，盲測）** | +0.880 | **+1.683** | < 0 | ❌ 見 C20.4 |
| V14 湧現 ε\*（盲測） | 761.8 | 761.8 | 100–1500 | ✅ 逐字不變 |
| V6a–f（盲測） | — | 完全相同 | — | ✅ |

`delta_ab`：**0.09184 → 0.24875**（×2.709）。皮質／小樑放大倍率由 3.6× 升為 **5.2×**。

**這不是新增自由度，是修復一個已知缺陷** —— 舊值本來就是對污染標的的擬合產物。且 `delta_ab` 現在由一個**算在正確腔室上**的標的釘住，辨識性反而更好。

### C20.4 ⚠️ 更正 C19.4 對 V10 的預測，並給出正確診斷

**C19.4 寫錯了。** 該節聲稱「`delta_ab` 夠大到滿足 V8 時 V10 也自動翻為正確負號 —— 一個參數同時解決兩者」。實測掃描顯示 **V10 對 `delta_ab` 非單調**：

| ×N | 1 | 2 | **2.709** | 3 | 3.5 | **4** | 6 | 8 |
|---|---|---|---|---|---|---|---|---|
| V8 | +4.47 | +9.09 | **+12.51** | +13.91 | +16.25 | +18.45 | +25.80 | +31.70 |
| V10 | +0.88 | +1.71 | **+1.68** | +1.41 | +0.75 | **−0.01** | −3.12 | −4.96 |

V10 先升後降，過零在 ×4 —— 而該處 V8 = +18.45%，遠出 11–14 帶。**兩者無法由 `delta_ab` 單獨同時滿足。** 我當初把 ×4 那列讀成落在標的帶內。

**V10 的正確診斷（三步排除）**：

1. **成分分解**：停藥後 12 個月的 +1.68% 幾乎全來自 **f_bm 仍在上升（+1.54%）**，礦化只貢獻 +0.14%。硬化蛋白**確實過衝**（1.072 vs 基線 1.0），但吸收僅升至 **C/C₀ = 1.017**，遠不足以逆轉。
2. **排除 SOST→RANKL 這條臂**：`lambda_S` 掃遍文獻全域（3–10）僅使 V10 由 +1.683 動到 +1.548；`K_L` 掃 0.5–30 亦僅 +1.5~+1.9。原因是 `L_RANKL = gS(S)/gS(1)`，`gS` 飽和時 `lambda_S` 在分子分母相消 —— 而更根本的是**過衝本身只有 +7.2%，那是 mechanostat 的後果，不是任何參數**。
3. **決定性檢查 —— 這是時間尺度問題，不是結構問題**：拉到 6 年，aBMD **確實會逆轉**：停藥後 **2.85 年達峰（+14.79%）**，第 6 年回到 12 個月水準以下。**模型是以 mechanostat 自己的時鐘在回收，約比臨床慢 5 倍。**

**結論**：V10 缺的是**抗體停藥的藥理反彈**（臨床上 CTX 會衝到基線以上），而非 mechanostat 的緩慢 SOST 回升。這是一個明確、可證偽、且指向具體機制的診斷 —— 比 v2.8「小樑範疇之外」的說法精確得多。

### C20.5 交付與影響

- 改動：`evalTargets`（V8/V10 移至小樑腔室、新增 `V8cort`）、新 `turnoverRate.m`、`trabecularParams`（閉式 `k_res`、切斷遞迴）、CSV `delta_ab`、`test_calibration`（**V8 由「只斷言方向」升級為斷言 11–14 帶**，並加兩項皮質臂護欄）、`E4_pharmacology`。**72 測試全過。**
- **V9 的弱形式仍成立**（減速由 8.7 倍變為 7.2 倍）；**V12 次相加由 7.7% 變為 9.9%**（`delta_ab` 變大使 SOST→Wnt 節點的競爭更明顯 —— 與 C18.4 的機制解釋一致）。
- **後續（P5h）**：加入 romosozumab 停藥反彈機制以達成 V10。這是目前唯一仍未達的定量標的。

---

## 附錄 C21：P5h —— 停藥反彈機制；**V10 達標，且是以 hold-out 身分達標** (2026-07-26)

**C20.4 的診斷（「缺的是抗體停藥的藥理反彈」）獲得證實。加入一個狀態 `A_reb` 後，V10 由 +1.68% 變為 −12.42%。關鍵在於它是對 CTX 標記校正的，不是對 BMD —— 所以 V10 仍是盲測預測。附帶收穫：V9 的強形式也一併重現。**

### C21.1 機制

新增**共用系統性狀態** `A_reb`：抗體長期暴露下，骨細胞對硬化蛋白訊號喪失的代償 —— **SOST 轉錄的上調**。

$$\frac{dA_{reb}}{dt}=\frac{s_{reb}\,u_{romo}-A_{reb}}{\tau_{reb}},\qquad \frac{dS}{dt}=\delta_S\,S_{set}\,(1+A_{reb})-\big(\delta_S+\delta_{ab}u_{romo}\big)S$$

抗體加的是**清除**，代償加的是**生產**，而**兩者的時鐘不同**：抗體停用是瞬間的，`A_reb` 以 $\tau_{reb}$ 衰減。停藥後清除項消失而升高的生產尚在 → 硬化蛋白過衝 → RANKL↑ → 吸收湧現 → BMD 回落。**這個時間差就是全部機制。**

狀態數：單站 16→**17**、雙站 29→**30**（`A_reb` 為系統性，非每站）。

### C21.2 結構性的隔離保證

**`A_reb` 在任何未用藥的模擬中恆為 0**，因為其驅動項是 `u_romo`。故 V1、V2、V7、V6、V14 **在結構上不可能被擾動** —— 不是「經檢查沒事」，是「原理上不會」。

實測驗證（先以 `sost_reb=0` 跑一次中性檢查，再以校正值複驗）：V1 7.1924、V2 1.1533、V7 1.1396、V7slope −0.2385、V14 761.8、**V6 盲測 25.7490 / 10.6579 / 1.2766** —— 與 v2.9 **逐字相同**。`test_rebound` 的第一項即斷言五個無藥情境下 `max|A_reb| == 0`。

### C21.3 ⚠️ 方法論核心：**校正到 V16，不是校正到 V10**

若把新參數擬合到 V10，V10 就從盲測變成擬合對象 —— 那會**摧毀本專案最有價值的一項證據**（附錄 C10.3）。

故新增獨立標的 **V16：停藥後 CTX 過衝**（臨床報告升逾基線約 +20–40%，帶 1.2–1.4），已登記於 `validation_targets.csv`（`holdout=FALSE`）並納入 `evalTargets` 的 `pass`/`resid`/`chi2`。**反彈機制對 V16（吸收標記）校正，V10（BMD 回落）因此仍是盲測預測。**

### C21.4 (`delta_ab`, `sost_reb`) 是相關對 —— 由 V16 打破簡併

`A_reb` **在治療期間亦會抵銷抗體**，故單獨提高 `sost_reb` 會壓垮 V8。兩者沿一條「V8 = 12.5%」的脊共同上移：

| `sost_reb` | 0.30 | 0.60 | 0.90 | 1.20 | **1.80** | 2.20 | 2.60 |
|---|---|---|---|---|---|---|---|
| `delta_ab`（沿脊擬合） | 0.496 | 0.733 | 0.967 | 1.197 | **1.650** | 1.948 | 1.986⚠️ |
| V16 CTX 過衝 | 1.084 | 1.156 | 1.216 | 1.256 | **1.305** | 1.326 | 1.342 |
| V10（盲測） | −2.10 | −5.32 | −7.82 | −9.72 | **−12.42** | −13.72 | −15.25 |

這與 C11 的 (K_P_sost, lambda_P) 同型 —— **但這次有解**：V8 只釘住脊的方向，**V16 釘住脊上的位置**。取 V16 = 1.3（臨床 1.2–1.4 之中點）得 `sost_reb = 1.80`、`delta_ab = 1.65048`。⚠️ `sost_reb ≥ 2.6` 時 `delta_ab` 已頂到上界 2.0，脊到此為止 —— 所選點在界內有餘裕。

**取值順序是先定 V16 標的、再讀 V10，不是反過來挑一個讓 V10 好看的點。**

### C21.5 結果

| 標的 | v2.9 | **v2.10** | 帶 | |
|---|---|---|---|---|
| V1 / V2 / V7 / V7slope | 7.192 / 1.153 / 1.140 / −0.238 | **完全相同** | — | ✅ |
| V6a–f（盲測） | 25.749 / 10.658 / 1.277 | **完全相同** | — | ✅ |
| V14（盲測） | 761.8 | **761.8** | 100–1500 | ✅ |
| **V8**（小樑） | 12.506 | **12.497** | 11–14 | ✅ 擬合 |
| **V16** CTX 過衝 | 1.017 | **1.305** | 1.2–1.4 | ✅ **新增，擬合** |
| **V10**（小樑，**盲測**） | **+1.683** | **−12.416** | < 0 | ✅ **達標，且為預測** |
| V10（皮質臂） | +0.267 | **−3.136** | — | 方向一致 |
| **V9** 強形式 | ❌ | ✅ | — | **本次一併解決** |
| V12 交互作用 | 次相加 7.7% | 次相加 7.8% | — | 不變 |

**V9 的意外解決值得說明**：`A_reb` 在**治療期間就追上抗體** —— 硬化蛋白由第 1 個月的 0.50 回升至第 9 個月的 1.03。這正是臨床上形成標記約 9–12 個月回到基線的自限性。v2.9 只能重現弱形式（上升減速），現在**強形式（療程內見頂回落）也成立**。**同一個機制同時解釋自限性與停藥反彈** —— 這是內部一致的圖像，而非兩個獨立補丁。

### C21.6 已知簡化（如實記錄）

1. **藥物是階躍函數**。真實 romosozumab 半衰期約 12 天，需數週清除；模型於 `stopDay` 瞬間關閉。後果：(a) 停藥瞬間硬化蛋白衝到 **3.65×** 基線，此峰值幅度被階躍誇大；(b) 模型的 CTX 峰出現在停藥後**約 3 週**，臨床約在 **3 個月**。**被校正的是過衝的幅度（V16），不是這個尖峰的形狀**，且 12 個月窗內的 BMD 積分效應對峰值時點不敏感。加入抗體 PK 是另一個機制，未做。
2. **`sost_reb = 1.80` 屬大幅上調**（SOST 生產 +180%），`confidence=low`。它是被 V16 釘住的推論值，不是實測的轉錄資料。若日後取得 romosozumab 治療下的 SOST mRNA/血清游離硬化蛋白時序，應直接複驗此值。
3. **V12 的評估窗改為第 12 個月（治療中）**。反彈存在後，第 24 個月的藥物臂已把增益吐回（−0.68%），此時做「負荷×藥物」對比量到的是反彈而非交互作用。改於治療中比較（亦即 Schulte 2026 的作法），結論不變：次相加 7.8%。

### C21.7 交付

新增 `A_reb` 狀態（`stateVector`/`baselineState`/`rhsFull`/`rhsTwoSite`/`siteRHS`/`osteocyteSignal`）、參數 `sost_reb`、`tau_reb`、標的 `V16`、`evalTargets` 之 V16、`tests/test_rebound.m`（6 項，首項即隔離保證）、`test_calibration` 之 V10 **由「記錄不斷言」升級為斷言**＋新增 V16 斷言、`E4_pharmacology` 更新。**79 測試全過。**

**至此 V1–V16 中，僅 V6e（髓腔方向，M7 的 `r_e` 缺口）與 V5b（骨層級符號，待 Fu 2025）未達。**

---

## 附錄 C22：P5i —— V6e 由 modeling drift 解決；**但它與 P1 第二句有真實張力** (2026-07-26)

**V6e 達標（髓腔 −2.88% → +13.44%），V6 六項全數在容差內，且未動 hold-out。代價是 P1 第二句在 aBMD 上不再成立 —— 這不是可以調掉的副作用，是模型內部一條真實的權衡，本附錄量化它並交由主持人裁決。**

### C22.1 先試而後棄的做法（負面結果，仍具資訊）

**首先嘗試「補上 mechanostat 的另一半」**：v2.10 以前只有形成受應變調控（`eta ~ A·d`），吸收完全不受（`xi` 只看面積與 TNF-α）。故負荷下形成在各表面同步上升、吸收不動 → 骨內膜淨增骨 → 髓腔縮小。看似顯然的修法是讓吸收成為形成的倒數（`xi ~ A·(d⁰/d)`，指數同為 1，以基線劑量正規化使基線分配不變）。

**實作後量測，方向相反**：網球側 `xi` 由 `[0.0500 0.4500 0.5000]` 變為 `[0.0534 0.4292 0.5173]` —— 吸收偏向**骨膜**而非骨內膜。

**原因**：MSIC 通道劑量在該應變區已**飽和**。骨膜表面本就位於飽和曲線更遠處，相對增益 `d/d⁰` 反而**小於**骨內膜，故倒數規則把吸收推向骨膜。這與 C6.3／C18.3 同源 —— 飽和一再翻轉本應「顯然」的方向。**該做法已回退**（`git checkout`），基線分配的中性檢查（差 0.000e+00）與此段診斷一併保留於此。

### C22.2 定案：modeling drift 是成對的

真正解釋 Haapasalo 的不是重塑分配，而是 **Frost 的 modeling drift 會使皮質整體外移，而非單純膨脹** —— 骨膜沉積伴隨骨內膜吸收。v2.10 只實作了骨膜那一半。

$$\frac{dr_p}{dt}=\dots+\text{modeling},\qquad \frac{dr_e}{dt}=\dots+\chi_{drift}\cdot\text{modeling}$$

新參數 `chi_drift = 1.0`（`source=derived`）：**純平移，保持皮質壁厚**。`chi_drift=0` 回到 v2.10。

**取 1.0 是原理選擇而非對 V6e 擬合** —— 純平移是 modeling drift 的教科書定義，V6e 因此仍是 hold-out。此項掛在 `modeling` 上，故**繼承其應變閾值**（校正情境 762 με 全部不觸發）與 P5e 的飽和界。

**標的中性實測**：V1 7.1924、V2 1.1533、V7 1.1396、V8 12.4971、V16 1.3051、V10 −12.4158、V14 761.75 —— 與 v2.10 相同。皮質壁厚 3.100 → 3.196 mm（平移而非侵蝕），`r_e < r_p` 全程成立。

### C22.3 結果：V6 六項全過，**但 P1 第二句失守**

| 指標 | v2.10 | **v2.11** | Haapasalo | |
|---|---|---|---|---|
| V6a BMC | +25.75 | **+11.44** | 14–27 | ✅ 容差內 |
| V6b Tot.Ar | +10.66 | **+11.74** | 16–21 | ✅ |
| V6c Co.Ar | +24.16 | **+10.04** | 12–32 | ✅ 容差內 |
| V6d I_max | +31.80 | **+23.57** | 27–67 | ✅ 容差內 |
| **V6e M.Cav** | **−2.88 ❌** | **+13.44** | 19 | ✅ **達標** |
| V6f Co.Dn | +1.28 | **+1.27** | −2–0 | ✅ |

**6/6 在容差內**（v2.10 為 5/6）。P2 仍乾淨成立（局部 +11.44% vs 全身 ±0.0000%）。

**⚠️ 代價**：阻力訓練 24 個月的 aBMD 增益由 **+5.565% 降為 +1.574%**，低於 P1 第二句的「> 4%」。掃 `chi_drift` 顯示**兩者沒有交集**：

| `chi_drift` | 0 | 0.5 | 0.7 | **1.0** |
|---|---|---|---|---|
| V6e 髓腔 | −2.88 ❌ | +4.72 ❌ | +8.05 ❌ | **+13.44 ✅** |
| P1 負荷邊際（aBMD） | +5.57 ✅ | +3.62 ❌ | +2.81 ❌ | **+1.57 ❌** |
| 同一對比（BMC） | +7.08 | +5.14 | +4.33 | **+3.09** |

V6e 需 `chi ≳ 0.85`，P1（aBMD）需 `chi ≲ 0.2`。**無解。**

### C22.4 這個張力的科學內容 —— 度量的選擇（第二次出現）

**近半是 DXA 的尺寸假影**：漂移使 $r_p$ 外移，而 $aBMD = A_g f_{bm}\rho/(2r_p)$。同一個對比以**骨量 BMC** 計為 **+3.09%**，以 aBMD 計僅 +1.57% —— **DXA 稀釋了 49% 的負荷反應**。這正是 `densitometry` 刻意保留的尺寸假影（其 docstring 明言「不是待修的 bug」），也是臨床上 DXA 低估骨膜適應的已知現象。

**另一半是負荷強度**：P1 說的是「**適當**負重方案」。現行 `resistance` 情境為力矩 ×3.0（2236 με）。提高至 ×4.0（2900 με，網球級、仍在生理帶內）：

| 方案 | 峰值應變 | ΔaBMD | **ΔBMC** |
|---|---|---|---|
| ×3.0, 40 循環, 3 天/週（現行） | 2236 με | +1.57% | +3.09% |
| **×4.0, 40 循環, 3 天/週** | 2900 με | +1.85% | **+4.50% ✅** |
| ×5.0, 100 循環, 5 天/週 | 3564 με | +2.81% | **+6.46%** |

**故 P1 第二句在「骨量 + 適當強度」下成立（+4.50%），在「aBMD + 現行情境」下不成立（+1.57%）。**

> **這是 C18.2 的同型問題再現**：P1 的第三句因**絕對 vs 差異中的差異**而翻轉，第二句因 **aBMD vs BMC** 而翻轉。**P1 的三個子句都對度量敏感，論文必須逐句寫明度量與情境，否則同一個模型可以支持相反的結論。** 這本身是值得寫進論文的方法學結果。

### C22.5 ⚠️ 待主持人裁決（本階段刻意不自行決定）

`chi_drift = 1.0` 是原理值且使 V6e 這個 hold-out 通過，故**保留為預設**。但它改變了一項旗艦預測的判定，以下屬科學判斷而非實作選擇：

1. **P1 第二句改以 BMC 陳述**（並把「適當負重」定義為 ≳2900 με），或
2. **保留 aBMD 陳述並下修門檻**（文獻上阻力訓練的 BMD 增益本就多在 1–3%，原「>4%」是計畫書寫作時的先驗而非資料），或
3. **`chi_drift` 降至 0.7 並接受 V6e 差一點**（髓腔 +8.05 vs 容差下限 +9）。

三者互斥。在裁決前，`E2_calciumLoading` 已同時回報 aBMD 與 BMC 兩種度量，`pass` 結構亦拆為 `loadInstruct_aBMD` / `loadInstruct_BMC`。

### C22.6 交付

`boneStructure`（drift 項）、參數 `chi_drift`、`test_twosite` 新增 2 項（V6e 斷言 + `chi_drift=0` 的歸因/稽核開關）、`E2_calciumLoading` 雙度量回報。**81 測試全過。**

**至此 V1–V16 僅 V5b（骨層級符號，待 Fu 2025）未達。**

---

## 附錄 C23：P1 第二句改以 BMC 陳述 —— 主持人裁決紀錄 (2026-07-26)

**C22.5 提出的三個互斥選項，主持人選 1：P1 第二句改以骨量 BMC 陳述，並把「適當負重」定義為峰值應變 ≈2900 με。本附錄記錄裁決、其理據、以及據此所做的全部改動。**

### C23.1 裁決

| | 舊（v1.0–v2.10） | **新（v2.11 起）** |
|---|---|---|
| 子句 2 度量 | aBMD | **BMC（骨量）** |
| 「適當負重」 | 未定義 | **峰值應變 ≈2900 με**，即新增的 `resistanceVigorous` 情境 |
| 門檻 | > 4% | **> 4%（不變）** |
| 實測 | +1.865%（aBMD）❌ | **+4.589%（BMC）✅** |

**未被選用的另兩個選項**（保留紀錄，因為它們是有效的替代讀法）：(2) 保留 aBMD 但下修門檻至文獻的 1–3%；(3) `chi_drift` 降至 0.7 並接受 V6e 差一點。

### C23.2 為何 BMC 是對的度量（而非為了讓數字好看）

1. **aBMD 在本模型中含一個已知且刻意保留的假影**。P5i 之後負荷會使皮質整體外移，而 $aBMD = A_g f_{bm}\rho/(2r_p)$ 除以投影寬度 $2r_p$。實測 **DXA 稀釋了 59%** 的負荷反應（BMC +4.589% vs aBMD +1.865%）。`densitometry` 的 docstring 早已明言此尺寸假影「不是待修的 bug」，其存在正是為了讓同一個模型能同時對照 DXA 文獻（V7、V8）與 pQCT 文獻（V6）。**以 aBMD 陳述子句 2，等於把一個度量假影誤讀為生物學結論。**
2. **子句 2 的命題是「負荷指令骨形成」，那是骨量命題**。BMC 直接量骨量；aBMD 量的是骨量除以尺寸，而負荷恰恰同時改變兩者。
3. **子句 1 仍以 aBMD 陳述，且這不是雙重標準**：鈣作用於礦物質而非尺寸，兩種度量在該對比下一致（實測久坐組 aBMD +0.450% vs BMC +0.461%），且對照文獻 Tai 2015 本就以 aBMD 報告。**度量應依命題與對照資料選擇，而非全文統一。**

### C23.3 為何「適當」必須釘死

原文的「適當負重方案」未定義強度，使子句 2 **無法證偽** —— 任何未達標的結果都可歸咎於「方案不夠適當」。現定義為峰值應變 ≈2900 με：

| 情境 | 峰值應變 | ΔBMC | ΔaBMD |
|---|---|---|---|
| `resistance`（中等，力矩 ×3.0） | 2236 με | +3.093% ❌ | +1.554% |
| **`resistanceVigorous`（力矩 ×4.0）** | **2949 με** | **+4.589% ✅** | +1.865% |

2949 με 仍低於 3000 με 的生理上限，且與菁英網球選手肱骨（2851 με）同量級 —— 是**劇烈但可達成**的方案，不是外推。**中等強度不足以支持子句 2，這一點如實寫入。**

### C23.4 P1 現況（三子句全部帶度量）

| 子句 | 度量與情境 | 實測 | 標的 | |
|---|---|---|---|---|
| 1 鈣為許可性 | aBMD，800→1500 mg/day，久坐 | **+0.470%** | < 1% | ✅ |
| 2 負荷為指令性 | **BMC**，2949 με，鈣充足 | **+4.589%** | > 4% | ✅ |
| 3 交互作用 | **BMC**，**絕對框架** | 缺鈣代價 **+0.939 個百分點**（占增益 19%） | > 0 | ✅ |
| 3′ 同一命題 | BMC，**差異中的差異** | **−0.536 個百分點** | > 0 | ❌ 符號反轉 |

負荷／鈣效應比 **9.8×**。

> **P1 整體成立，但成立的方式帶三個限定條件**：子句 2 只在 BMC 與 ≈2900 με 下成立；子句 3 只在絕對框架下成立；子句 1 在兩種度量下皆成立。**這三個限定不是弱點，是本模型能給而現象學模型給不出的精確性。**

### C23.5 交付

- 新情境 `scenarioLibrary("resistanceVigorous")`（力矩 ×4.0、軸向 ×3.0、40 循環、0.5 Hz、循環間休息 10 s、3 天/週）。已納入 `test_closedloop` 的情境清單。
- 計畫書 §0 子句 1–3 全面改寫（各自帶度量）、§5 之 E2 列、§10 論文敘事補充。
- `E2_calciumLoading`：負荷臂由 2 個增為 3 個；**BMC 成為子句 2、3 的主要度量**，aBMD 並列以顯示稀釋；`pass.loadInstruct` 為改寫後的判定，`pass.loadInstruct_aBMD` 保留舊寫法的判定以供對照；圖改為 BMC 長條圖（含 P1 門檻線）＋ BMC vs aBMD 對比。
- **81 測試全過**（新情境未改變任何標的）。
...
## 附錄 C24：四個從未被執行的標的（v2.13, 2026-07-26）

### C24.1 起因

投稿前逐節稽核驗證主張時，比對 `data/validation_targets.csv` 與程式碼，發現 **V7b、V11、V13、V15 從來沒有任何實驗評估過**。四者的機制都確實建了，原始碼註解甚至寫明「這是 V11／V13／V15 的機制」（`specificSurface.m`、`osteocyteSignal.m`、`surfaceAllocation.m`、`estrogenTNF.m`），但 `evalTargets` 不算它們，E0–E6 也不碰它們。

也就是說，我們一直在一張二十個標的的表上工作，其中四個只有機制、沒有量測。新增 `experiments/E7_qualitativeTargets.m` 把它們跑完。

### C24.2 結果：一過三敗

| | 結果 | |
|---|---|---|
| **V11** 流失–回復不對稱 | ✅ **通過** | 比值 **25.8**（標的 >3）。半量流失 139 天，回復同量 3582 天；十年只取回 51.5 % |
| **V7b** 血清鈣孟德爾隨機化 | ❌ **符號錯** | 箝制血清鈣 +2 % → aBMD **+0.124 %**（應不上升） |
| **V13** 硬化蛋白雙重作用 | ❌ **幅度不足** | 方向對（RANKL 1.00→1.29、OPG 1.00→0.44），但破骨活化 **1.42 倍**，文獻約 7 倍 |
| **V15** 停經幾何 | ❌ **後半失敗** | 皮質變薄 ✅，但骨膜**向內** −0.0022 mm，未重現骨徑增寬 |

**V11 不得寫成雙穩態證據**（承 C16.2）：回復是單調且仍在進行的，這正是 P3 否證的旁證而非反證。

### C24.3 V13 的天花板是結構性的，不是調參問題

`L_RANKL` 對 S 走 Hill 項，`K_L = 0.5` 而基線 S = 1，故在基線就已飽和 2/3。可達的最大倍率為

  (1 + λ_S) / (1 + λ_S/1.5) = 4/3 ≈ 1.333

**與 λ_S 取值無關**。要達成 ~7 倍必須讓 `K_L` 遠高於基線硬化蛋白 —— 是結構修改，不是重擬。（`lambda_S = 3` 原本就是照著「約 7 倍」設的，但基線正規化把它吃掉了。）

### C24.4 V15 的「通過」是 C15.4 假影再現

十年 E2=0.1 的模擬給出骨膜外擴 **+5.75 mm**，看起來壯觀通過。但該run **第 334 天就逸出線性彈性域**；+5.75 mm ≈ modeling 速率上限 1.93 μm/day × 3650 天，是飽和界在頂著跑，不是生理。

在有效域內（第 300 天）讀，骨膜是**向內**的，標的失敗。

> **紀律**：任何幾何結論在報告前都必須先看 `out.validity.ok`。P5e 加這個守衛就是為了這件事，而它這次確實攔下了一個會被誤報為成功的結果。

### C24.5 附帶發現：血清鈣擺動 15 %，且回歸測試給了假保證

E7 順帶量到：飲食鈣 400 → 1500 mg/day，血清游離鈣由 **1.107 → 1.291 mmol/L（15 % 擺動）**。生理上血清鈣被恆定在約 ±2 %；`S1 Table` 對 `n_renal` 的說明自己也寫著「約 2 %」。**模型沒有做到它自己文件宣稱的事。**

機轉很清楚：`absFcn` 的被動項 `a_p_abs * I_Ca` **對攝取量線性且不飽和**（0.15 × 1500 = 225 vs 0.15 × 800 = 120），完全淹沒會飽和的主動項；唯一的防線是腎臟的 `(Ca_s/Ca_s_0)^n_renal`，於是 (225/120)^(1/8) = 1.081，恰好就是量到的 +7.6 %。

而**參數表宣告了 `renal_k` 與 `renal_Ca_th`，`calciumPTHvitD.m` 從來沒有讀過它們** —— 當初規劃的可飽和小管重吸收沒有實作，`Renal_0` 是直接由基線平衡導出的。

**回歸測試為何沒攔下**：`test_systemic/testSerumCalciumIsTightlyRegulated` 的註解寫「within a few percent」，但斷言是 `verifyLessThan(dev, 0.15)` —— **界是 15 %**。它原本是 v1.8 為了防 55 % 失控而寫的「失控防護」，此後卻一直被當成恆定性檢查來讀。已改寫其註解與訊息據實說明（**門檻刻意保留 15 %，因為收緊它是改模型而不是改測試**）。

### C24.6 對 P1 的影響（重要，但不推翻）

飲食鈣在本模型中有一部分是透過超生理的血清鈣擺動抵達骨骼的。但由此產生的 PTH 變化（800→1500 mg/day 為 **−14.5 %**）在臨床上是合理量級，故 V7 的效應量仍與 Tai 2015 相符。

**結論：效應量對，路徑被誇大。** 這是缺陷不是簡化，已寫入稿件 Results 新節與 Limitations，不得淡化為「模型簡化」。

### C24.7 待辦 P5k：修可飽和腎小管重吸收

依序：(1) 讓被動吸收項飽和或設上限；(2) 實作 `renal_k`／`renal_Ca_th` 的可飽和小管重吸收；(3) 收緊 `testSerumCalciumIsTightlyRegulated` 至 ~3 %；(4) 重跑 V1/V2/V7/V7slope/V8/V16 校正並複驗 V6/V10/V14 盲測。

**注意**：這會動到校正，因此不是投稿前該做的事 —— 現在的正確處置是如實揭露，已完成。

---

---

## 附錄 C25：V8 / V10 / V16 稽核（v2.13, 2026-07-26）

### C25.1 稿件的 V10 數字是錯的

稿件（含摘要、README、三個語言版本）寫 **−13.967 %**。以 v2.13 參數重跑 `evalTargets(p, holdout=true)`：

```
V10  −12.416 %
```

**−13.967 是舊值，已全數更正為 −12.416。** 一併更正三個隨參數微漂的數字：V8cort +2.443 → **+2.437**、負荷單獨 +0.817 → **+0.810**、併用 +2.904 → **+2.898**、次相加 10.9 → **10.8 %**。

> **教訓**：稿件的每個數字都必須能由當前 HEAD 重跑得到。送出前應把 Results 的數字對著一次 `evalTargets` 全表逐一核對。

### C25.2 V10 的判準太鬆，且幅度並不吻合

判準只是 `r.V10 < 0` —— **只有方向**。稿件卻寫「重現臨床上觀察到的快速流失」，讀起來像幅度也對上了。實際軌跡：
...
| 停藥後 12 個月 | **−12.416 %** |
| **吐回增益的比例** | **111.8 %** |
| **24 個月時 vs 基線** | **−1.47 %（跌破基線）** |

臨床對照（McClung 2018, PMID 29694685，本次新增為文獻 23）：轉為安慰劑後 BMD **"returned toward pretreatment levels"** —— **趨近**基線，不是穿過。

**結論：方向、速度、可逆性三者重現；終點多跌約 1.5 個百分點。** 已在稿件中明說判準只有方向、並如實給出 111.8 % 與 −1.47 %。歸因為抗體被建模為階躍函數（清除在 `A_reb` 仍處平台時驟停）。

### C25.3 V8 通過，但 5.1 倍放大是無對照的數字

V8 = +12.497 %，落在 11–14 % 內 ✅，且**是校正標的**（`delta_ab` 對它擬合），稿件已揭露。

但 **5.1 倍的小樑／皮質放大從未對照任何實證數字**，卻被寫成一項結果。最接近的臨床對照是 McClung 2018 的腰椎 +15.1 % vs 全髖 +5.4 %（約 3 倍）——**但全髖是皮質＋小樑混合部位，我們的皮質腔室是純骨幹**，本就該更大，兩者不可等價比較。已在稿件中明講「沒有直接的實證對應物，不當成已驗證的結果呈現」。

### C25.4 V16 通過，但原本沒有任何引註

V16 = 1.305 倍，落在 1.2–1.4 內 ✅，且已揭露為校正標的。

問題在於 **「文獻報告的 1.2–1.4 區間」原本一個引註都沒有**，V10 的「臨床上觀察到的快速流失」也沒有。兩者現在都指向 McClung 2018（β-CTX 轉安慰劑後 "increased above baseline"）。

> **這是一類值得單獨記住的缺陷**：把一個數字寫成「文獻報告的」，卻沒有指出是哪一篇。稽核時要把每個「reported / observed / clinically」都當成待查的引註缺口。

---

---

## 附錄 C26：V1 / V2 / V3 / V7 稽核 —— 稽核完成，並查出系統耦合的單向缺陷（v2.13, 2026-07-26）

至此 **V1–V16 全部標的皆已用「主張／判準／誠實比對」過完一遍。**

### C26.1 三個標的原本沒有來源

| | 原本 | 現在 |
|---|---|---|
| **V1** 成人骨轉換 5–10 %/yr | 無任何引註 | **Parfitt 2002**（PMID 12052445）所引述的皮質慣用區間 **3–10 %/yr** |
| **V2** 廢用流失 1.0–1.5 %/mo | 無任何引註 | **Spatz 2012**（PMID 22767636）：承重部位 **1–2 %/月** |
| **V3** 廢用硬化蛋白上升 48.8 % | 無判準、無引註 | 同上，Spatz 2012 逐時點數值 |

> **這已是本輪稽核第二次撞到同一件事**（C25.4 是 V10/V16）。**凡寫「文獻報告的」「臨床觀察到的」而未指名出處者，一律視為引註缺口。**

### C26.2 V1 通過，但標的設在被引用範圍的偏快端

Parfitt 引述的皮質區間是 **3–10 %/yr**；我們的標的取 **5–10**，是其上半部，而模型值 **7.19 %/yr** 又落在標的的上半。三重偏上。已在稿件中據實寫成「這是對著一個設在被引用範圍偏快端的標的而通過」。

> ⚠️ Parfitt 該文主旨是**檢討**「小樑轉換必然高於皮質」被無條件套用，不是為那組數字背書。**不可寫成「Parfitt 認為皮質骨轉換為 3–10 %/yr」。**

### C26.3 V2 的欄位名稱與實際計算的量不一致

`data/validation_targets.csv` 的欄位是 `dBMD_per_month`，但 `evalTargets` 算的是 **BMC**：

```matlab
r.V2 = -100 * (oB.dens.BMC_L(end)/oB.dens.BMC_L(1) - 1) / 6;
```

實測影響可忽略：廢用時骨膜幾乎不動（180 天 −0.002 mm），BMC 與 aBMD 相差 **0.003 個百分點/月**。**是標籤錯，不是數字錯。** 但同樣兩個量在負荷情境下相差 **59 %**（P1 的 DXA 稀釋），所以這個標籤不能繼續放著。

### C26.4 V3 方向對、形態錯

| | 第 28 天 | 第 60 天 | 第 90 天 |
|---|---|---|---|
| Spatz 2012 實測 | +29 % | **+42 %（達峰）** | +22 % |
| 本模型 | **+48.8 %** | +48.8 % | +48.8 % |

模型在第 28 天就到頂並**完全持平**。上升與大致峰值幅度重現，**衰減沒有**：硬化蛋白在數日內即平衡到新的力學劑量，之後沒有任何適應性回落。

### C26.5 ⚠️ 最重要的發現：骨→血的耦合在數值上是可忽略的

Spatz 同一批受試者：**副甲狀腺素第 28 天 −17 %、第 60 天 −24 %，尿鈣全程顯著升高** —— 典型失用症圖像。

本模型（bedrest 第 60 天）：

```
Ca_s  +0.0059 %      PTH  −0.012 %      1,25D  −0.006 %
破骨 C +10.26 %      成骨 B −50.26 %
```

**細胞層級動得很厲害，全身層級完全沒動。**

**機轉**：`imbalance = (Abs + phi_res*vRes − phi_form*vForm − Renal) / Abs_0`，其中 `phi_res = phi_form = 0.10`，而 `Abs_0 = 0.15×800 + 0.1 ≈ 120.1`。骨通量項相對於正規化基準是 **0.1/120.1 ≈ 1/1200**。再經 `n_renal = 8` 的腎臟壓制，Ca_s 只動 ~0.01 %。

**參數表對 `phi_res` 的原始理由是錯的**：註解寫「骨鈣池 vs 每日周轉，應遠小於 1」——那把**存量**比值當成**通量**比值。生理上骨吸收釋出的鈣通量與淨腸道吸收通量是**同一量級**，不是千分之一。

**後果**：稿件（摘要與方法）宣稱全身鈣模組「**雙向耦合**」。就方程式而言為真，就數值而言**回程實質不存在** —— 全身模組在驅動骨骼，而非與之交換。任何需要骨骼移動全身鈣的現象（失用性高血鈣為最）都在本參數組能力之外。已寫入 Results 與 Limitations。

### C26.6 待辦 P5k（擴充）：鈣模組兩半一起修

C24.7 只列了腸道／腎臟那一半。現在是兩半：

1. 被動吸收項飽和或設上限；
2. 實作 `renal_k`／`renal_Ca_th` 的可飽和小管重吸收；
3. **把 `phi_res`／`phi_form` 提到與 `Abs_0` 同量級**（或改用未正規化的通量平衡）；
4. 新增標的：廢用時 PTH 須下降 15–25 %；
5. 重跑校正並複驗 V6／V10／V14 盲測。

**第 3 項會改變 M6↔M8 的耦合強度，屬於結構性改動，不是投稿前該做的事。** 現階段的正確處置是如實揭露，已完成。

---

---

## 附錄 C27：P5k —— 鈣模組兩半一起重建（v2.14, 2026-07-26）

C24.7 與 C26.6 把兩個缺陷分別列為待辦，並註明「屬結構性改動、不是投稿前該做的事」。主持人裁決：**兩半一起修**。

### C27.1 診斷：同一個模組在兩個相反方向上壞掉

| | 症狀 | 病因 |
|---|---|---|
| **正向（飲食 → 血鈣）太鬆** | 400→1500 mg/day 使血清鈣擺動 **15 %**（生理約 2 %） | 主動吸收項被寫成**分率**而非通量，只貢獻 0.08 % 的吸收 —— **受 PTH／calcitriol 調控的那一臂實質不存在**；而腎臟只是裸的冪次律 `(Ca_s/Ca_s_0)^n_renal` |
| **反向（骨 → 血鈣）太弱** | 廢用 180 天 PTH 只動 **−0.01 %**（臨床 −17~−24 %） | `phi_res`/`phi_form` = 0.10，而通量失衡是用大它一千倍以上的 `Abs_0` 正規化 |

**兩者同源**：模組把「分率」與「通量」混用，於是唯一真正有量綱意義的項（被動吸收）獨大。

### C27.2 重建後的三個結構決定

1. **主動吸收成為真正會飽和的通量**，且由 calcitriol 閘控：`A_act = a_a_abs * I_Ca_0 = 280 mg/day`。被動分率由 0.15 降為 **0.08**，使受調控的一臂承擔基線吸收的多數。**這一臂就是飲食鈣的負回饋**：攝取↑ → 血鈣↑ → PTH↓ → 1,25D↓ → 主動吸收↓。
2. **腎臟改為閾值形式**，終於用上宣告已久卻從未被讀取的 `renal_k` 與 `renal_Ca_th`：

   `Renal = renal_k * max(Ca_s − Ca_th, 0)`，`Ca_th = renal_Ca_th + renal_lambda_P * gap * (P−1)`

   **`renal_k` 是導出的，不是自由參數**：`renal_k = (Abs_0 + phi_res − phi_form)/gap`。整個腎臟防禦的陡度因此壓縮成**單一個生理選擇 —— 閾值離基線多近**。取 `renal_Ca_th = 1.176`（低於 Ca_s_0 約 2 %），等價於「約 98 % 的濾過鈣被再吸收」，這正是排泄成為「兩個大數之差」而極度敏感的原因。
   > ⚠️ PTH 對閾值的位移必須用 **gap** 縮放，不能用 Ca_th 本身：gap 只有 Ca_th 的 2 %，若用後者，幾個百分點的 PTH 變動就會把整個防禦掀翻。
3. **`phi_res`/`phi_form` = 350 mg/day**（成人骨鈣周轉量級），兩者相等故基線交換為零。

另外 **`n_P` 由 4 提高到 16**。這是**形狀約束不是擬合**：典型 PTH–鈣 S 形曲線在約 1.0–1.4 mmol/L 之間由近最大降到近最小（±17 % 內超過十倍），對數斜率約 −8；而中點斜率即 n_P/2，故 n_P ≈ 16。舊值 4 的斜率是 −2，要重現失用症的 20 % PTH 抑制就得讓血鈣上升 10 %，那是明確的高血鈣症。

### C27.3 兩個診斷在重擬之前就已通過

| | 修前 | 修後 | 參照 |
|---|---|---|---|
| 血清鈣擺動（400→1500 mg/day） | 15.15 % | **1.61 %** | 生理約 2 % |
| 臥床第 60 天 PTH | −0.01 % | **−19.9 %** | −24 %（Spatz 2012） |

### C27.4 重擬：surrogateopt 幾乎沒用，因為問題近乎可分離

`surrogateopt` 320 次評估只把 fval 由 1.185 降到 1.044，V7 與 V16 仍不達標。原因是這個問題**近乎可分離**：V1←k_res（閉式）、V2←K_S、V7←(lambda_P, K_P_sost)、V8/V16←(delta_ab, sost_reb)。改用逐項掃描立刻定位。

> **教訓**：六維盲搜之前，先問這個目標函數是不是可分離的。是的話，掃描比代理模型有效得多。

`sost_reb` 於本版**加入自由參數清單**（P5h 時是手動擬合的，但 V16 一直在目標函數裡，等於讓 delta_ab 扛兩個標的）。

### C27.5 ⚠️ 新發現的結構性衝突：V7 與 V16 搶同一個 lambda_P

把骨→血那一臂變成真的之後，兩個標的開始**經由 PTH→RANKL 往相反方向拉**：

- **V7 要 lambda_P 大**：飲食鈣↑ → PTH↓ → RANKL↓ → 護骨。
- **V16 要 lambda_P 小**：停藥後吸收暴增 → 骨鈣湧入 → PTH↓ → RANKL↓ → **把吸收過衝壓下去**。

實測（K_S=1.18, K_P_sost=60）：

| lambda_P | 2 | 4 | 6 | 8 | 10 |
|---|---|---|---|---|---|
| V7 | 0.428 | 0.591 | 0.669 | 0.715 | 0.745 |
| V16 | 1.228 | 1.198 | 1.184 | 1.175 | 1.170 |

**V7 需要 ≥ 約 7，V16 需要 ≤ 約 4 —— 沒有任何值同時滿足。** `sost_reb` 也失去對 V16 的槓桿（上調 33 % 只讓 V16 動 0.019），`tau_reb` 同樣無效（且會把 V8 拉走）。

**這是舊的解耦模組看不見的衝突**，是把耦合做真之後才浮現的。取 lambda_P = 6 為折衷。

### C27.6 最終狀態

```
k_res 3.66151e-07 | K_S 1.18 | K_P_sost 100 | lambda_P 6 | delta_ab 1.72 | sost_reb 1.81
k_form 3.583641e-07（導出）

V1 7.034 ✓ | V2 1.196 ✓ | V7 +0.705 ✓ | V7slope −0.140 ✓ | V8 +12.160 ✓
V16 1.1836 ✗（差 0.016，宣告容差 0.15）
HOLD-OUT: V10 −11.471 ✓ | V14 761.8 ✓ | V6a–f 六項全過 ✓
測試 84 通過 / 0 失敗
```

**盲測全數存活於一次結構重寫加重擬之後** —— 這是本次最強的結果。

**`K_P_sost` 落在上界 100**，但這是**可解讀的極限而非任意釘死**：大值代表 PTH 對 SOST 的抑制弱，擬合等於在要求「這條路徑不存在」，而那正是 Lemaire 與 Pivonka 的選擇。該臂與 V7 反向，關掉它才讓 V7 在血鈣被正確調控後仍能勉強達標。

### C27.7 ⚠️ 兩個踩雷點

1. **`getDefaultParams` 不會重算 `k_form`** —— 它直接讀 CSV。改動 `k_res` 之後 CSV 裡的 `k_form` 就成了**過期的導出值**，我因此一度得到與掃描不符的 V7（0.610 vs 0.705）。**改 `k_res` 必須同時手動改寫 `k_form`**；`test_calibration/testFormationIsBoneMassBalanced` 是攔截網。
2. **V7 現在只是勉強達標（0.705 對下界 0.7）**。舊值 1.140 是**靠超生理的血鈣擺動撐出來的**。血鈣被正確調控後，鈣→PTH→骨這條路只能產生約 +0.7 %，落在 Tai 2015 區間的最下緣。**這其實強化 P1（鈣是許可性的）**：鈣的效應比我們原先以為的還小。

### C27.8 未竟

- `n_renal` 已標為 superseded，保留供對照。
- V16 的衝突若要真正解決，需要停藥反彈機制不經由 PTH→RANKL 這條共用路徑 —— 屬未來工作。

---

---

## 附錄 C28：P5l —— 採用 Currey 的彈性律指數（v2.16, 2026-07-28）

### C28.1 結果摘要：一個採用、一個否決，而否決本身才是結果

| | 原值 | Currey 1988 式 (10) | 結論 |
|---|---|---|---|
| `nu_E`（對礦化度） | 1.0（assumed） | **2.55** | ✅ **採用** |
| `kappa_E`（對 f_bm） | 2.5（assumed） | **3.13** | ❌ **試了，然後撤回** |

### C28.2 為何 kappa_E = 3.13 不能用：Currey 量的是**緻密骨**

Currey 1988 的樣本是「compact bone of 18 species」，體積分率接近 1。把該指數外推到小樑腔室的 f_bm = 0.12：

```
0.12^(3.13 − 2.5) = 0.12^0.63 = 0.26
```

表觀模數乘以 0.26 → **99.8 MPa 變成 26.2 MPa**，低於椎體小樑骨實測的 50–300 MPa。後果立刻出現：

- 小樑基線 24 個月漂移 **3.81 %**（門檻 0.5 %）
- 小樑基線**逸出線性彈性域**（最高 10547 με）
- 用藥情境亦逸出
- **三個 `test_trabecular` 測試連帶 `test_rebound` 一項失敗**

> **這反過來說明了一件重要的事**：`kappa_E = 2.5` **不只是「假設值」** —— 它是讓**同一條彈性律跨越兩個數量級孔隙度**的折衷值。稿件裡那句「free consistency check：E_ref 與 kappa_E 共用且未調，在 f_bm = 0.12 給出 99.8 MPa，落在實測 50–300 MPa 內」**正是靠這個折衷才成立的**。換成緻密骨的真實指數，那個一致性檢查就垮了。

**上下界仍維持放寬後的 2.0–6.0**，以記錄「緻密骨文獻值明顯高於 2.5」這件事。

### C28.3 為何 nu_E = 2.55 可以用

礦化度在兩個腔室都坐在各自的基線（`rho_min/rho_min_0 = 1`），故提高其指數**不會擾動小樑模數**。實測確認：小樑 E_app 仍為 99.8 MPa、無漂移、彈性域完好。

採用後全部標的幾乎不動（V8 由 +12.160 變 +12.394，仍在帶內；V10 −11.454；V14 761.8 不變），**84 測試全過**。這是一次「把假設值換成有出處的值」的純收益。

### C28.4 正確的修法（待辦 P5m）

單一 `kappa_E` 橫跨皮質（f_bm ≈ 0.95）與小樑（0.12）本來就是勉強的。物理上這是兩種材料：

- **緻密骨**：Currey 的冪次律，指數約 3.1（雙變數）
- **小樑骨**：開孔泡沫，Gibson–Ashby 標度 E ∝ ρ²，指數約 2

**建議做法**：讓 `trabecularParams` 覆寫 `kappa_E`（如同它已覆寫 `K_tau`、`eps_model_star` 的作法），皮質用 3.13、小樑用 2.0，並重擬。**這會動到校正，且需要重新檢查 V8/V10 與 P3。**

> 這也修正了稿件目前的一句宣稱：「The elastic law therefore holds across two orders of magnitude of porosity without adjustment.」—— 嚴格說，它之所以成立，是因為指數取了一個**兩邊都不對**的折衷值。這句話在 P5m 之後需要重寫。

---

---

## 附錄 C29：P5m —— 小樑腔室覆寫 kappa_E（v2.17, 2026-07-28）

### C29.1 做法

C28 的診斷是：**一個 `kappa_E` 服務不了兩種材料**。P5m 依 `trabecularParams` 既有的覆寫慣例（它已覆寫 `K_tau`、`eps_model_star`、`M_L_0` 等）補上一行：

| 腔室 | `kappa_E` | 依據 |
|---|---|---|
| 皮質 | **3.13** | Currey 1988 雙變數式，緻密骨 |
| 小樑 | **2.00** | Gibson–Ashby 開孔泡沫標度 E ∝ ρ² |

閉環恆等式 d ln ε / d ln f_bm 因此在皮質為 **−3.13**、在椎體為 **−2.00**。

### C29.2 結果：全部通過，且不需重擬任何參數

| | v2.16 | **v2.17** |
|---|---|---|
| 小樑 E_app @ f_bm=0.12 | 99.8 MPa | **288.0 MPa**（實測帶 50–300） |
| V1 | 7.034 | 7.034 |
| V2 | 1.196 | **1.185** |
| V7 | +0.705 | **+0.716** |
| V8 | +12.394 | **+12.042** |
| V16 | 1.1836 | 1.1830（仍未達，衝突照舊） |
| V10（盲測） | −11.454 | **−11.380** ✅ |
| V14（盲測） | 761.8 | **786.8** ✅（仍在 300–1500） |
| V6a–f（盲測） | 六項全過 | **六項全過** ✅ |
| 測試 | 84/0 | **84/0** |

**`delta_ab` 不需要動** —— V8 自己落在帶內。這是少見的「純結構修正，零重擬」。

### C29.3 值得記住的三點

1. **盲測再一次全數存活**（V6a–f、V10、V14），這是第三次結構改動後仍如此（P5i、P5k、P5m）。
2. **P3 未受影響**：`test_bifurcation` 三項全過，單穩態結論不變。C28 曾預期「回饋更強會強化 P3」，皮質端確實如此。
3. ⚠️ **小樑 288 MPa 靠近實測帶（50–300）的上緣**。Gibson–Ashby 的 E/Es = C(ρ/ρs)² 裡我們等於取 C = 1；文獻常見 C ≈ 0.5–1。若日後要調，`C` 是比指數更該動的旋鈕，且不影響指數的物理依據。

### C29.4 ⚠️ 稿件需要再一次數字對帳

P5m 動到的稿件數字（相對 v2.14 對帳後的版本）：

- **d ln ε_p / d ln f_bm = −2.5000**（與 −κ_E 吻合）→ 皮質 **−3.13**，且此句須說明兩腔室指數不同
- **小樑 E_app 99.8 MPa** → **288.0 MPa**（那句 free consistency check 要重寫）
- **「The elastic law therefore holds across two orders of magnitude of porosity without adjustment」→ 這句話現在是錯的**，必須改為「指數依腔室而異，皮質取自緻密骨量測、小樑取自開孔泡沫標度」
- V14 761.8 → **786.8**；V6a–f 六項數值；V8、V10、V2、V7 微幅變動

**稿件目前停在 v2.14 的數字**，尚未反映 P5l（nu_E）與 P5m。

---

---

## 附錄 C30：Lerebours 2015 —— `S_v(f_bm)` 確實是同一個病灶（v2.19, 2026-07-28）

### C30.1 原文怎麼說

Lerebours 等人明白寫出高骨體積分率區的行為：

> BS/TV = a · (1 − BV/TV)^**b**，其中多項式與 harmonic mean 擬合皆給 **b = 1**，而他們所提出並實測的平方根擬合給 **b = 0.5**。

擬合優度：**平方根 r² = 0.84** > Martin 五階多項式 0.82 > Fyhrie harmonic mean 0.77。且「**BV/TV ≥ 0.9 時**，資料點以比任何整數次多項式更接近平方根的分數次冪收斂到 0」。

### C30.2 我們的指數錯得比 kappa_E 更離譜

本模型：`S_v = S_v_max · C · f^s1 · (1−f)^s2`，**s2 = 3**。

| | 高 BV/TV 區的 b |
|---|---|
| Lerebours 實測（平方根擬合） | **0.5** |
| Martin 多項式／harmonic mean | 1 |
| **本模型** | **3** |

**我們的指數是實測值的六倍**，甚至是另外兩個候選的三倍。而**皮質腔室正坐在 f_bm = 0.95** —— 正是他們量測的那個區。

**量化後果**：在 f = 0.95 處

```
本模型   d ln S_v / d ln f_bm = s1 − s2·f/(1−f) = 1 − 3(19) = −56
Lerebours                     ≈ −0.5·f/(1−f)   = −9.5
```

**比表面積對孔隙度變化的反應約陡了六倍。**

基線骨轉換不受影響（`S_v_hat` 在基線被正規化為 1），但**所有經由 `S_v` 的變化率都被誇大**，其中包括 **V11 的流失–回復不對稱** —— 而 `specificSurface.m` 的註解正是把該不對稱歸因於這個函數。

### C30.3 這就是 kappa_E 的同一個病灶

| | `kappa_E`（P5m 已修） | `s2_Sv`（待修） |
|---|---|---|
| 形式選在哪個區 | 折衷值 2.5 | 圓頂峰值設在 f_bm = 0.25（**小樑區**） |
| 實際用在哪 | 皮質 f_bm = 0.95 | 同上 |
| 實測值 | 3.13（緻密骨） | **0.5**（皮質高 BV/TV） |
| 我們的值 | 2.5（偏低） | **3（偏高六倍）** |

**同一種錯誤，方向相反**：`kappa_E` 我們取得太低，`s2_Sv` 我們取得太高。兩者都源於「一個函數硬跨兩種材料」。

### C30.4 額外發現：這個關係是**個體專一**的，不是內在的

原文標題就是這句。逐個體擬合 r² = 0.87–0.98，遠優於全體合併的 0.84；且與**性別與孔洞密度**相關，與身高、體重、年齡無關。

> 這對稿件有意義：我們把 `S_v(f_bm)` 當成一條普遍曲線，但原文說它**因人而異**。這比「皮質與小樑不能共用」更強 —— 連同一類骨在不同個體間都不能共用。

### C30.5 P5n：試了 b = 0.5，**未採用**，而未採用的理由才是結果

依 P5m 的作法實作：皮質 `s2_Sv = 0.5`（Lerebours 實測），`trabecularParams` 覆寫回 3.0（Martin 的圓頂本來就擬合在小樑區）。

**機制上完全成功**：皮質基線的 `d ln S_v / d ln f_bm` 由 **−56** 變成 **−8.51**，與 Lerebours 隱含的 −9.5 相當接近。

**但 V2 變成無法達成。**

| K_S | 1.18 | 1.10 | 1.00 | 0.90 | 0.70 | 0.50 | 0.30 | **0.10（下界）** |
|---|---|---|---|---|---|---|---|---|
| **V2** | 0.434 | 0.489 | 0.565 | 0.645 | 0.800 | 0.901 | 0.937 | **0.944** |

**把 K_S 掃到宣告下界，V2 仍飽和在 0.944 %/月，始終進不了 1.0–1.5 帶。** 這不是調參失敗，是結構結論：

> **本模型的廢用流失，先前有相當部分是靠被誇大六倍的比表面積反應撐出來的。** 換上實測指數之後，**必須另有機制供給那份流失**，而我們現在沒有。

（順帶：K_S 下修時 **V16 反而回到帶內**（K_S = 0.9 給 1.2189），且 V7 升到 +1.192、V8 = 13.5 —— 除了 V2 之外，其他標的在低 K_S 反而更好。）

### C30.6 為何**版本庫維持 s2_Sv = 3**

與 P5l 撤回 `kappa_E` 的情形**不完全相同**，這點要說清楚：

- **P5l**：採用實測值產生**物理上不可能**的狀態（小樑模數低於實測帶、逸出彈性域）。撤回是唯一選項。
- **P5n**：採用實測值產生一個**物理上沒問題**的模型，只是一個校正標的差 0.056（宣告容差 0.5，其實在容差內）。

**所以嚴格說，0.5 才是比較有依據的值** —— 3 已經被證實是實測值的六倍。維持 3 是**工程上的保守選擇而非科學上的判斷**：要負責任地採用 0.5，需要一次完整重擬（並找出補上廢用流失的機制），而不是把 `K_S` 釘在下界了事。

**這一條要誠實記著**：我們現在 shipping 的是一個**已知錯六倍**的指數。稿件 Limitations 必須寫出來。

### C30.7 待辦 P5n′（重新定義）

1. 找出被誇大的 S_v 反應原本在代償什麼 —— 廢用流失的缺口從何補。候選：骨細胞凋亡對吸收的直接驅動、或 `xi` 表面分配在廢用時的偏移。
2. 補上之後才採用 `s2_Sv = 0.5`，並重擬。
3. 複驗 V1／V2／**V11**／P3 與三組盲測。
4. 稿件 Limitations 同步改寫，並加上 Lerebours 的「**個體專一**」論點。

---

---

## 附錄 C31：P5n′ 起步 —— 排除了一個候選，並修正一個我自己的錯誤推論（v2.22）

目標：找出「被誇大六倍的 `S_v` 反應」原本在代償什麼，補上之後才能採用 `s2_Sv = 0.5`。

### C31.1 骨細胞凋亡**不受力學驅動**（已確認）
...
**只看雌激素。** 函式簽章連力學劑量都沒收。臥床情境 E2 = 1，所以**整段廢用期間凋亡率都停在基線**。

這與已知生物學不符：卸載會誘發骨細胞凋亡，而凋亡骨細胞（及其鄰居）釋出 RANKL 造成標定性吸收。**這仍是缺口的首要候選。**

### C31.2 但「感測增益」這條路已被排除

`zeta` 是 `(n_ot/n_ot_0)^zeta` 的指數，**基線恆為 1，故改它不動基線** —— 是最乾淨的探針。在 `s2_Sv = 0.5` 下掃描：

| zeta | 1.5 | 2.5 | 4.0 | 5.0 |
|---|---|---|---|---|
| **V2** | 0.434 | 0.434 | 0.435 | 0.435 |
| V7 | +0.780 | +0.934 | +1.350 | +1.954 |
| V8 | +12.73 | +14.13 | +15.93 | +18.28 |

**V2 完全不動，而 V7／V8 動得很大。** 所以「把感測增益調大」補不上廢用流失。

### C31.3 ⚠️ 我第一次給的理由是錯的，量測推翻了它

我原本推斷 `zeta` 無效是因為「廢用時 `n_ot` 不動」（既然凋亡不受力學驅動）。**實測不是這樣**：

```
180 天臥床：n_ot 1.0000 -> 0.7929  (−20.7 %)
            感測增益 (n_ot/n0)^zeta = 0.7060
```

`n_ot` **確實掉了 20.7 %** —— 因為吸收會帶走骨細胞（`gammaEff * v_res * n_ot`），而廢用時吸收上升。訊號有在動。

**所以 `zeta` 無效的原因在下游，不在上游。** 最可能是 SOST 的力學抑制 Hill 項（`h_S = 3`）在廢用時已經飽和：硬化蛋白已上升 61.2 %，再降低感測也擠不出更多。

> **這一條要記著**：我差一點把一個錯的機制解釋寫進計畫書。**排除法給的是「哪條路不通」，不會自動給你「為什麼不通」** —— 後者要另外量。

### C31.4 量測結果：SOST 力學項在廢用時**完全飽和**（v2.22）

`fMech(Y) = 1 / (1 + (Y/K_S)^h_S)`，`K_S = 1.18`、`h_S = 3`。

| | 基線 | 廢用 180 天 |
|---|---|---|
| YAP/TAZ | 1.0000 | **0.0785**（−92.2 %） |
| `fMech` | 0.6216 | **0.9997** |

**0.9997 是絕對上限 1.0 的 99.97 %。** 把 Y 一路壓到 **零**，`fMech` 只從 0.9997 變成 1.0000 —— **額外倍率 1.00，等於什麼也買不到**。

### C31.5 這一刀砍掉了首要候選

**任何經由感測作用的機制，都補不上廢用流失的缺口** —— 包括**力學驅動的骨細胞凋亡**，因為它同樣是 `n_ot` → 感測 → SOST，而 SOST 的力學臂已經頂到天花板。

C31.3 的更正因此是關鍵的：若我當時沒回頭量 `n_ot`、就把「訊號不動」寫成結論，這一步會被誤導去實作凋亡，然後發現它同樣無效 —— 而且**很可能會誤以為是實作錯了**。

### C31.6 附帶發現：整條力學感測路徑的動態範圍只有 1.61 倍

基線 `fMech = 0.6216`、上限 1.0，所以**這條路徑從基線到完全卸載總共只能把 SOST 生成拉高 1.61 倍**，而廢用把這 1.61 倍用光了。

要加大餘裕就得降低 `K_S`（讓基線離飽和更遠）—— **但 `K_S` 正是被 V2 釘住的參數**。循環相依。

> 這是對 M4 的一項結構性陳述，值得單獨記著：**mechanostat 的感測臂是有硬性上限的，而廢用已經打到上限**。這也解釋了為何 `K_S` 對 V2 那麼敏感（1.10 → 1.91 %/月、1.24 → 0.91）—— 它其實是在移動飽和點，不是在調增益。

### C31.7 P5n′ 的下一步（已再次收斂）

缺口只能由**繞過 SOST 的路徑**供給。剩餘候選：

1. **凋亡骨細胞直接釋出 RANKL** —— 新增 `n_ot → RANKL` 項，不經 `S`。這是文獻上「標定性吸收」的機制，也是目前最有依據的一條。
2. **`lambda_xi` 在廢用時的表面分配偏移** —— 已有參數，但目前只由 TNF-α（即雌激素）驅動，與凋亡同型的問題。
3. 降低 `K_S` 以加大感測餘裕 —— **但它被 V2 釘住**，需與重擬一起做。

**建議先做 1**：它是新增而非重擬，可獨立驗證，且不會動到現有校正。

---

---

## 附錄 C32：P5n′ 實作規格 —— `n_ot → RANKL`（尚未執行，v2.22 撰寫）

C31.4 證明 SOST 力學臂在廢用時飽和到 99.97 %，故缺口只能由**繞過硬化蛋白**的路徑供給。以下是可照著執行的規格。**下一輪請一次做完，不要做一半。**

### C32.1 為何是這一項

凋亡／瀕死骨細胞直接釋出 RANKL 造成標定性吸收，是文獻上的既有機制。在本模型中它**繞過 `S`**，因此不受 `fMech` 飽和的限制 —— 這正是 C31.5 所要求的性質。

`n_ot` 在廢用 180 天掉 **20.7 %**（C31.3 實測），所以驅動訊號確實存在且夠大。

### C32.2 程式改動（四處，已逐一確認）

**1. `src/signal/osteocyteSignal.m`** —— 加一個因子到 RANKL：
...
% 改為
gA = @(n) 1 + p.lambda_apop * max(0, 1 - n / p.n_ot_0);   % 凋亡缺口 -> RANKL
alg.L_RANKL = (gS(S) * gP(P_pth) * gE(E2) * gA(n_ot)) ...
            / (gS(1) * gP(1) * gE(p.E2_0) * gA(p.n_ot_0));
```

`gA(n_ot_0) = 1`，故**基線嚴格不變** —— 這是關鍵性質，讓 V1／V7／V8／V14 在結構上不受影響（與 `delta_ab`、`sost_reb` 同樣的設計）。

**2. 簽章**：`osteocyteSignal(s, D_eff_hat, P_pth, E2, u_romo, A_reb, p)` → 增一個 `n_ot`。建議插在 `E2` 之後（生理上同屬狀態量）。

**3. 呼叫端只有兩處**（已 grep 確認）：
- `src/model/siteRHS.m:52` —— `s.n_ot` 就在手邊（同一個狀態結構）
- `experiments/E7_qualitativeTargets.m:132` —— V13 的掃描，傳 `p.n_ot_0` 即可（該處要的是基線）

**4. 新參數 `lambda_apop`**（`data/parameters_literature.csv`，module M4）：
`value` 待擬合、`lower` 0、`upper` 10、`source` calibrated、`confidence` low。

### C32.3 校正程序

`lambda_apop` **只在 `n_ot < n_ot_0` 時生效**，故它與 `delta_ab` 一樣是「情境專屬」的：基線與所有非廢用情境逐字不變。因此**單參數掃描即可，不必全域重擬**：

1. 設 `s2_Sv = 0.5`（Lerebours 值 —— 這整件事的目的）
2. 掃 `lambda_apop` 使 **V2 落回 1.0–1.5**
3. `k_form` 必須重新導出並寫回 CSV（見踩雷點 #14）

### C32.4 驗收清單（缺一不可）

- V1 7.03、V7、V8、V14 **應逐字不變**（`gA` 基線為 1）—— 若變了，代表 `gA` 的正規化寫錯
- **V2 落回 1.0–1.5**
- **V11 必須複驗** —— 它是廢用／回復不對稱，直接受此項影響
- **V10、V6a–f 三組盲測維持通過**
- **P3 單穩態維持**（`test_bifurcation` 三項）
- 84 測試 0 失敗

### C32.5 若失敗，那也是結果

若 `lambda_apop` 在其上界仍補不上 V2，那就與 C30.5 同型：**廢用流失的缺口不在 RANKL 這條路上**，下一個候選是 `lambda_xi` 的表面分配偏移。**照 C30 的方式如實記錄，不要把它調到極端值硬塞。**

---

---

## 附錄 C33：P5n′ 執行結果 —— 這條路走得通，但它買不起（v2.23, 2026-07-30）

C32 的規格照做了，**而且加做了它自己漏掉的另一半**。兩個機制都已建成、量測完畢，並**以 0 值出貨（inert）**。

### C33.1 先確認：新增項在基線上是嚴格中性的

`gA(n) = 1 + lambda_apop · max(0, 1 − n/n_ot_0)`，加入 `L_RANKL` 的分子與分母。

以 `lambda_apop = 0` 對照 HEAD，全部標的最大偏差 **4.7e-11** —— 而那個殘差來自我把參考值以 `%.10f` 抄寫時的截斷，不是模型。**四處程式改動在關掉時是位元等價的**，所以之後量到的每一個變化都能歸因於機制本身，不是被挪動的基線。

這一點已鎖進 `tests/test_apoptoticArm.m`（5 項，總數 84 → **89**）。

### C33.2 它確實補得上 V2 —— 這是 C32 的預測，成立了

皮質 `s2_Sv = 0.5`（Lerebours 實測），`trabecularParams` 覆寫回 3.0（Martin 圓頂本來就擬合在小樑區）。皮質基線 `d ln S_v/d ln f_bm` 由 **−56** 變成 **−8.50**，與 C30.5 記錄的 −8.51 一致，也接近 Lerebours 隱含的 −9.5。

| `lambda_apop` | 0 | 1 | 3 | 5 | 6 | 7.5 | 10 |
|---|---|---|---|---|---|---|---|
| **V2**（帶 1.0–1.5） | 0.434 | 0.536 | 0.751 | 0.942 | **1.023** | **1.128** | **1.268** |

`lambda_apop = 0` 給 **0.4343**，精確重現 C30.5 的 0.434。**V2 在 `lambda_apop ≈ 6` 首次以實測指數進入帶內** —— C30.5 說「必須另有機制供給那份流失」，這個機制找到了。

### C33.3 但代價是 P1 的平台，而平台是硬要求

| | HEAD（s2=3） | s2=0.5, λ=0 | s2=0.5, **λ=6** |
|---|---|---|---|
| V1 | 7.034 | 7.034 | 7.034 |
| **V2** | 1.185 ✅ | 0.434 ❌ | **1.023 ✅** |
| V7 | 0.716 ✅ | 0.780 ✅ | 1.731 ✅ |
| **V7 平台斜率** | −0.140 ✅ | −0.139 ✅ | **−0.543 ❌** |
| V8 | 12.042 ✅ | 12.042 ✅ | 12.042 ✅ |
| V16 | 1.183 ❌ | 1.183 ❌ | 2.234 ❌ |
| V10（盲測） | −11.38 | −11.38 | **−27.59** |
| V14（盲測） | 786.77 | 786.77 | 786.77 |
| **chi²** | **3.44** | 13.80 | **92.96** |

`validation_targets.csv` 對 V7 寫得很直接：「**The PLATEAU SHAPE is a hard requirement**」。而那個平台就是 **P1 子句 1 的內容** —— 鈣的效果是非累進的。`lambda_apop = 6` 讓低鈣臂在第 2.5 年還在以 **−0.54 %/yr** 下滑（HEAD 是 −0.14），**鈣缺乏變成累進性的**。

**這不是「多輸一個標的」，是拿全文頭號預測去換一個參數的正確性。**

V1／V8／V14 三項如 C32.4 所要求**逐字不變**（`gA` 基線為 1）。V16 與 V10 也大幅惡化，但兩者證據力較弱：V16 的 1.2–1.4 是我們自訂的帶（§0b.四），且 HEAD 本來就沒達成。

### C33.4 機轉：這一項根本不是「廢用專屬」的

C32.3 假定它「與 `delta_ab` 一樣是情境專屬的」。**這個假定是錯的，而錯在哪裡是這一輪最有用的產出。**

`gA` 的驅動量是骨細胞缺口。但在本模型裡，**唯一會製造缺口的是吸收本身**（`gammaEff · v_res · n_ot`）—— 因為凋亡項只看雌激素，不看力學（C31.1）。所以 `gA` 實際上是

> **吸收 → RANKL → 更多吸收**

的正回饋，**在每一個情境裡都開著**，而不是只在廢用時開。`delta_ab` 靠 `u_romo` 這個外生開關保證情境專屬；`gA` 沒有這種開關。

低鈣臂 **2.5 年**的骨細胞缺口只有 **0.25 %**，廢用臂 **180 天**就有 **20.7 %** —— 缺口本身相差 80 倍。但 V2 的視窗是 6 個月，V7 的視窗是 2.5 年，**複利把那 0.25 % 放大到足以摧毀平台**。缺口大小的懸殊救不了它，因為兩個標的比的不是同一段時間。

### C33.5 於是我把 C32 漏掉的另一半也建起來（P5n″）

若缺口改由**卸載**而非吸收製造，它就會真的是廢用專屬的。這在 C31.5 曾被排除，但**那個排除只對感測路徑成立**：C31.5 說力學驅動凋亡無效，理由是它走 `n_ot → 感測 → SOST`，而 SOST 力學臂飽和。**`gA` 存在之後，凋亡有了一條不經 SOST 的出口** —— 所以 C31.5 的結論在 C32 落地後已部分失效，必須重測。

實作：`apop = delta_ot_0 · (E2_0/E2) · (1 + lambda_ot_mech · max(0, 1 − D_eff_hat))`，同樣寫在**劑量缺口**上，基線恆為 1。

**特異性確認了** —— 低鈣臂幾乎不動，廢用臂大幅加深：

| `lambda_ot_mech` | 0 | 3 | 10 | 30 | 100 | 200 |
|---|---|---|---|---|---|---|
| 廢用 180 d 的 `n_ot` | 0.793 | 0.786 | 0.770 | 0.728 | 0.608 | **0.490** |
| 低鈣 2.5 yr 的 `n_ot` | 0.9975 | — | — | 0.9973 | 0.9965 | 0.9940 |

**但它救不回來，而且是往反方向走的。** 每個 `lambda_ot_mech` 都重新校正 `lambda_apop` 使 V2 = 1.20：

| `lambda_ot_mech` | 0 | 30 | 100 | 200 |
|---|---|---|---|---|
| 所需 `lambda_apop` | 8.70 | 7.73 | 6.39 | 5.42 |
| **V7** | 7.95 | 6.93 | **11.34** | **41.87** |
| **V7 平台斜率** | −3.60 | −2.95 | −5.18 | **−20.95** |
| V16 | 2.39 | 2.35 | 2.29 | 2.24 |
| chi² | 411 | 314 | 733 | **10423** |

把凋亡誘導拉到 **200 倍**，`lambda_apop` 只從 8.7 降到 5.4，而 **V7 反而從 7.95 惡化到 41.87**。

> **`lambda_ot_mech` 的出處是 `assumed`，不是文獻。** 卸載誘發骨細胞凋亡的倍率我們**沒有查證過任何原文**，200 倍純粹是為了「把這條路推到極限也證明它不通」而取的探測值 —— 不是主張生理上會有 200 倍。依 §0 的紀律，這裡不寫任何未經核對的倍率。若要正式主張這條路已被排除，需先取得凋亡誘導倍率的實測出處（見 C33.8 第 2 項）。

原因是規模：`k_ot(n_ot_max − n_ot_0)/n_ot_0 = 0.1 /day`，而 `delta_ot_0 = 0.001 /day` —— **基礎凋亡只佔骨細胞移除的 1 %，掩埋與吸收佔 99 %**。所以文獻量級的凋亡誘導幾乎不動 `n_ot`；要動就得把倍率推到 O(100)，而那時整個系統已經走進強非線性區（V7 對 `lambda_apop` 變成非單調）。

> ⚠️ **這個 1 % 是兩個 assumed 參數（`delta_ot_0`、`k_ot`）算出來的。** 若日後其中任何一個取得出處，**C33 的結論必須重算，不能繼承**。已寫成 `testApoptosisIsAOnePercentArmOfOsteocyteTurnover`。

### C33.6 附帶發現：實測指數讓廢用流失變成**不可逆**

C32.4 要求複驗 V11，而 C30 正是把 V11 的不對稱歸因於 `S_v`。180 天臥床後恢復日常負重、追蹤 10 年：

| | 臥床流失 | 10 年後相對基線 | V11 判準（比值 > 3） |
|---|---|---|---|
| **HEAD（s2 = 3）** | −7.11 % | **−2.59 %**（回復 63.5 %） | 22.13 ✅ |
| **s2 = 0.5** | −2.61 % | **−4.67 %**（回復 **0 %**） | **Inf** ✅ |

實測指數下**流失較小，十年後卻掉得更深** —— 因為指數放平時，**回復被壓抑得比流失更多**。C30 說 V11 的不對稱靠這個函數撐著，這裡看到的是它撐的方式：撐的是「回不來」那一半。

> ⚠️ **同時這是一個「判準因為錯誤理由而通過」的實例**：V11 只要求比值 > 3，而回復為零時比值是 **Inf**，形式上通過得最漂亮。**看斷言，不要看通過與否** —— 與 C24.5 血鈣 15 % 那個假保證同型。

### C33.7 裁決：不採用，兩個機制以 0 值出貨

`s2_Sv` **維持 3**，`lambda_apop = 0`，`lambda_ot_mech = 0`。

理由不是 C32.5 預期的那個。C32.5 猜的是「缺口不在 RANKL 這條路上」；**實際上缺口就在這條路上，而且補得起來** —— 不採用是因為在本模型的結構裡，這條路無法與其他情境隔離，於是它換來的是 P1 的平台。

**用一個已知錯六倍的參數，換全文頭號預測的成立 —— 這個交換在稿件送出前不划算。** 這與 C30.6 的判斷一致，但現在它有了實測基礎：**維持 3 不再只是「工程上的保守選擇」，而是一個量過價格之後的決定。**

程式保留而非刪除，因為它現在是**被測過的路**：任何人要重開只要改一個 CSV 欄位，而 `test_apoptoticArm.m` 的最後一項會在改動當下擋下來、指回這一節。

### C33.8 下一步（依價值排序，取代 C32.5 的清單）

1. **`lambda_xi` 的表面分配偏移** —— C32.5 原本的次選，仍未測。它有機會避開 C33.4 的病灶：`lambda_xi` 由 TNF-α 驅動，而 TNF-α 不是吸收的產物。**但它與凋亡同型的問題仍在（只看雌激素，不看力學），先確認這一點再動手。**
2. **`delta_ot_0` 與 `k_ot` 找出處** —— C33.5 的 1 % 是整條凋亡路線的死因，而它建立在兩個 assumed 值上。這是**最便宜的一步**：兩個數字若有出處，C33.5 與 C31 的排除都要重算。
3. **重新檢視「必須同時滿足 V2 與 V7 平台」是否為真限制** —— V2 的帶（1.0–1.5）比原文（Spatz 1–2）更窄（§0b），而 V7 的平台是硬要求。若 V2 放寬到原文的 1.0–2.0，`s2_Sv = 0.5` 需要的 `lambda_apop` 不會變小（下界仍是 1.0），所以**這條路不通** —— 已在 §0b 記過，此處確認。
4. **顯式小樑穿孔** —— 與 C33.6 的「不可逆」發現直接相關：實測指數下回復為零，而連續 `f_bm` 描述抓不到模板喪失。這可能不是巧合。

---

---

## 附錄 C34：為 `delta_ot_0` 與 `k_ot` 找出處 —— 找到的是一個 500 倍的內部矛盾（v2.23, 2026-07-30）

C33.8 把這一項列為第一優先，理由是「C33.5 判死凋亡路線的那個 1 % 是兩個 `assumed` 值算出來的，任一個有出處就要重算」。查了。**結論比預期的重要，而且方向相反。**
...
```
dn_ot/dt = k_ot·v_form·(n_ot_max − n_ot) − [γ_eff·v_res + δ_ot(E2)]·n_ot
```

基線平衡把 `γ_eff` 釘死，所以 **`k_ot` 不是一個獨立的「掩埋速率」，它設定的是整個骨細胞族群的周轉率**：

```
k_ot·(n_ot_max − n_ot_0)/n_ot_0 = γ_eff + δ_ot_0 = 0.1 /day
```

即**骨細胞平均停留時間 10 天**。要比對的就是這個數。

### C34.2 不需要文獻就看得出來：模型自相矛盾 514 倍

骨細胞是**隨著它所在的那塊骨被吸收**才離開組織的。所以骨細胞族群的分數移除率**必須等於**骨的分數周轉率。

| | |
|---|---|
| 骨每天被吸收的分數（由 V1 = 7.034 %/yr 換算） | **1.93 × 10⁻⁴ /day** |
| `γ_eff`（模型中吸收帶走骨細胞的速率） | **9.90 × 10⁻² /day** |
| **比值** | **514 倍** |

**模型說骨每年換 7 %，卻說骨細胞每年換 3650 %。** 這一條用不到任何外部資料 —— 它是模型跟自己打架。

### C34.3 文獻確認是哪一邊對

**Buenzli & Sims 2015**（Bone 75:144–150, doi:10.1016/j.bone.2015.02.016）在摘要裡就給了兩個絕對數字：成人骨骼共約 **420 億**個骨細胞，每天補充 **910 萬**個。

```
2.167 × 10⁻⁴ /day   →   停留時間 4615 天 = 12.6 年
```

與 V1 換算出的 1.93 × 10⁻⁴ /day **同一量級**（差 12 %，而兩者是完全獨立的路徑）。**所以錯的是 `k_ot`，不是 V1。** 模型出貨值快了 **462 倍**。

> **一個保守方向的提醒**：Buenzli & Sims 是全骨骼平均，含周轉快的小樑骨；皮質骨幹更慢，所以對我們這個腔室，真值只會**更低**，差距只會**更大**。

由此可導出：`k_ot = 2.167×10⁻⁴ · n_ot_0/(n_ot_max − n_ot_0) = **1.083 × 10⁻³ /day**`。

### C34.4 `delta_ot_0` 沒找到直接出處 —— 但也不需要了

沒有查到「基礎骨細胞凋亡速率」的實測值。**而量下去發現這個問題本身是錯的問題**：

| 凋亡佔骨細胞移除的比例 φ | 1 % | 10 % | 25 % |
|---|---|---|---|
| V7 | 0.6255 | 0.6210 | 0.6135 |
| V2 | 1.1810 | 1.1810 | 1.1810 |

**在修正後的 `k_ot` 之下，φ 掃 25 倍，V7 只動 0.012、V2 完全不動。** 因為 `n_ot` 一旦變成慢變數，它在任何情境裡都幾乎不動，**兩個移除臂怎麼分根本不重要**。

> C33.8 問錯了問題。它問「1 % 這個**比例**對不對」，但真正壞掉的是**整體速率**。**比例是分子分母之比，而分子分母是同一個錯誤參數算出來的** —— 這也是為什麼那個 1 % 看起來自洽。

最接近的可測代理是**空陷窩比例**：**Power et al. 2001**（Calcif Tissue Int 69:13–19, doi:10.1007/s00223-001-0013-6）量老年股骨頸皮質，陷窩被活骨細胞佔據的比例在 **12.4 %–99.2 %** 之間，且與局部骨轉換相關（周轉快 → 骨齡輕 → 佔據率高）。這說明在慢周轉的皮質裡，原位死亡**絕不可忽略**。但**模型不追蹤空陷窩，這不是同一個量** —— 依 §0 紀律，記為佐證，不代入。

### C34.5 修正 `k_ot` 的後果：兩個標的掉出帶外，兩個變好

| | HEAD（k = 0.5） | 文獻值（k = 1.083e-3, φ = 10 %） |
|---|---|---|
| V1 | 7.034 ✅ | 7.034 ✅ |
| V2 | 1.185 ✅ | 1.181 ✅ |
| **V7** | 0.716 ✅ | **0.6255 ❌**（帶 0.7–1.8） |
| V7 平台 | −0.140 ✅ | −0.122 ✅ |
| **V8** | 12.042 ✅ | **10.572 ❌**（帶 11.9–14.7） |
| V16 | 1.183 ❌ | 1.128 ❌ |
| **V10**（盲測） | −11.38 | **−8.59（過衝變小，更接近臨床的「趨近基線」）** |
| V14（盲測） | 786.77 ✅ | 786.77 ✅ |
| 24 個月漂移 | −0.0147 % | −0.0140 % |
| **V11 十年回復** | 63.5 % | **144.2 %** |
| chi² | **3.44** | 8.36 |

**模型在物理上完全沒問題**（漂移正常、`validity.ok = 1`、V11 比值 14.79 仍 > 3）。這是 **P5m 型**而非 P5l 型的情況：不是產生不可能的狀態，而是需要一次重擬。

> **P3 不受影響（解析可證）**：把 `k_ot`、`γ_eff`、`δ_ot_0` 同乘一個因子，`n_ot* = k·v_f·n_max/(k·v_f + γ·v_r + δ)` 的因子直接約掉，**不動點完全不變**。只有 φ 的改變會微幅移動不動點（因為 γ 乘 `v_res` 而 δ 不乘）。所以**單穩性結論不因時間尺度修正而動搖**。

### C34.6 這反過來加強了 C33，並廢掉 C32.1 的前提

C32.1 寫：「`n_ot` 在廢用 180 天掉 20.7 %，所以驅動訊號確實存在且夠大。」

**在修正後的速率下，那個缺口是 2.3 %（`n_ot` = 0.9772），不是 20.7 %。**
...
1. **C32.1 的前提是那個 514 倍錯誤的產物。** 「訊號夠大」是假的。
2. **C33 對凋亡 RANKL 臂的否決因此更強**，不是更弱 —— 缺口小一個數量級，這條路只會更沒有力量。
3. **C31.3 的「`n_ot` 確實掉了 20.7 %」也是同一個假象。** 那一節當時是拿來更正一個錯誤推論的（C31.3 標題就是「我第一次給的理由是錯的」）—— 現在看，**更正後的理由也建立在壞掉的參數上**。原始結論（`zeta` 無效）仍然成立，因為 C31.4 的 SOST 飽和量測與 `k_ot` 無關。

### C34.7 `lambda_ot_mech` 的倍率也查到了 —— 而且小得多

C33.5 明講「200 倍是探測值，不是文獻主張；要正式排除這條路必須先取得倍率出處」。取得了：

**Basso & Heersche 2006**（Bone 39:807–814, doi:10.1016/j.bone.2006.04.014）：大鼠後肢懸吊 2 週，小樑幹骺端**凋亡骨細胞百分比上升 66 %**，即 **1.66 倍**，對應 `lambda_ot_mech ≈ 0.66`。

**C33 需要 200 才讓這一項起作用，文獻給 0.66 —— 差 300 倍。**

> 對應的限制要講清楚：大鼠非人、小樑幹骺端非皮質骨幹、14 天非 180 天，且量的是**某一時點的凋亡分數**而非速率常數。即使全部放寬，**出處把值推向讓這條路更弱的方向**，所以 C33 的否決現在站在有出處的基礎上。

人體方向性佐證：**Rolvien et al. 2020**（J Bone Miner Res 35:1343–1351, doi:10.1002/jbmr.3970）在長期臥床老年女性的股骨皮質發現骨細胞密度與小管連結度下降，且型態與停經後骨鬆**可區分**。

### C34.8 最重要的一筆：有人已經用實驗做過 C33

**Plotkin et al. 2015**（J Biol Chem 290:18934–18942, doi:10.1074/jbc.M115.642090）標題就是結論：

> *Inhibition of Osteocyte Apoptosis Prevents the Increase in Osteocytic RANKL but **Does Not Stop Bone Resorption or the Loss of Bone Induced by Unloading***

他們用 IG9402（不抑制吸收的雙磷酸鹽類似物）擋掉骨細胞凋亡，**骨細胞 RANKL 的上升被擋住了，骨還是照樣流失**。摘要的 highlight 直接寫：「Prevention of apoptosis and reduction of osteocytic RANKL are not sufficient to stop unloading-induced bone loss」、「RANKL from non-osteocytic sources contributes to bone loss induced by reduced mechanical forces」。

**這是 C33 的獨立實驗對照，而且結論一致**：凋亡 → RANKL 這條臂是真的，但**它不是廢用骨流失的載體**。C33 是從模型內部推到這個結論的（它不是情境專屬、買不起 P1 的平台），Plotkin 是從動物實驗推到的（擋掉它，骨照樣掉）。

> **這一筆要寫進稿件。** 我們的負面結果現在有文獻背書，而不只是「我們試了但沒採用」。

### C34.9 裁決與下一步

**`k_ot` 維持 0.5 出貨，但這是一個記錄在案的缺陷，不是一個參數選擇。** 已用 `testOsteocyteRemovalIsInconsistentWithBoneTurnoverByKnownFactor` 釘住 514 這個比值 —— **那個測試預期會在有人修好它的那天失敗**，訊息裡寫明了那正是預期的修法。

**P5o（下一輪的第一件事）**：採用 `k_ot = 1.083e-3` 並重擬。

1. V8 的旋鈕是 `delta_ab`，而它是情境專屬的（踩雷點 #5），所以**修 V8 不會動到 V7** —— 這是單參數重擬。
2. **V7 沒有專屬旋鈕**，它是 P5k 之後就坐在區間下緣的（§0b 三）。0.6255 距下界 0.0745。這是 P5o 真正的難點。
3. 重擬後必須複驗三組盲測（V6a–f、V10、V14）與 P3 三項。**V10 在此改動下本來就變好（−11.38 → −8.59），這是加分而非代價。**
4. 圖表與稿件數字會變 —— 這是本輪未動它的主要原因（稿件目前與 HEAD 完全一致）。

---

## 附錄 C35：P5o 執行 —— 修正 `k_ot`、重擬，並在校正器裡找到兩個缺陷（v2.24, 2026-07-30）

### C35.1 修法是結構性的，不是換一個數值

沒有把 `k_ot` 設成 1.083e-3 了事。**改成導出量**（`src/analysis/osteocyteBurialRate.m`），與 `k_form` 同一個模式：

```
γ_eff := turnoverRate(p)/100/365
k_ot  := (γ_eff + delta_ot_0)·n_ot_0/(n_ot_max − n_ot_0)
```

**這讓 C34 的矛盾不可能再發生**，而不只是暫時不存在。比值現在是精確的 1.00000（原為 514）。已接進 `calibrate`、`probeTargets`、`identifiability`、`trabecularParams` 四處 —— **`k_res` 一動 `k_ot` 就必須跟著動**，否則矛盾當場復發。

**附帶好處**：小樑腔室自動拿到它自己較快的骨細胞周轉（2.85e-3 對皮質 1.07e-3），不需要第二個參數 —— 因為 `trabecularParams` 本來就把 `k_res` 校到 20 %/yr。

`delta_ot_0` 由 0.001 改為 **2.14e-5**（原值比修正後的**總**移除率還大，會把 `γ_eff` 逼成 0）。取 10 % 的凋亡佔比，對應 90 % 陷窩佔據率。**C34.4 已證明這個比例不敏感**（1 %→25 % 只動 V7 0.012）。

### C35.2 校正器裡的兩個缺陷，都是「宣稱與實作不符」

**缺陷一：V16 從來不在目標函數裡。** `calibrate.m` 的 docstring 從 v2.14 起就寫「`sost_reb` → V16」與「V16 sits in the objective」，但目標函數只有 V1、V2、V7、V7slope、V8。**`sost_reb` 因此是一個沒有任何標的在約束它的自由參數**，這也解釋了 V16 為何從沒達成。

**缺陷二（量出來才知道）：把 V16 加進目標函數不是修法。** 實測 `sost_reb` 的槓桿：

| `sost_reb` | 0 | 1.0 | 1.84 | 2.5 | 3.0 |
|---|---|---|---|---|---|
| **V16** | 1.143 | 1.159 | 1.129 | 1.156 | 1.173 |
| **V8** | **63.60** | 29.87 | 11.99 | 4.17 | **0.47** |

**它對 V16 的全域變動只有 0.04，對 V8 卻是 63.6 → 0.47。** 所以它根本不是 V16 的旋鈕，而是**第二個 V8 旋鈕**，與 `delta_ab` 沿 V8 脊簡併 —— 而 V16 破不了這個簡併。加 V16 進目標只做了一件事：把 `lambda_P` 往下壓，**犧牲有出處的 V7（Tai 2015）去追一個我們自己編的 1.2–1.4 帶**（§0b.四）。實測代價：V7 0.663 → 0.635，fval 0.602 → 1.219。

**正確的修法是把 `sost_reb` 移出自由清單**（固定在 P5h 的 1.81），讓 `delta_ab` 獨自承載 V8。自由參數 6 → **5**。

> **通則，值得單獨記住**：在因為「某個標的應該能辨識它」而把參數放進自由清單之前，**先量那個標的推不推得動它**。這次的 docstring 講了一個合理的故事，而那個故事從來沒有被量過。

### C35.3 重擬結果：五個參數全部在內部

`surrogateopt`、160 次評估、3 workers、fval **0.6017**。

| | HEAD (v2.23) | P5o (v2.24) | |
|---|---|---|---|
| `k_res` | 3.66151e-07 | **3.815954e-07** | |
| `K_S` | 1.18 | **1.201052** | |
| `K_P_sost` | **100（上界）** | **91.39865** | 離開上界 |
| `lambda_P` | 6 | **6.980592** | |
| `delta_ab` | 1.72 | **1.764614** | |
| `sost_reb` | 1.81（自由） | 1.81（**固定**） | |
| `k_ot` | 0.5 | **1.111198e-03（導出）** | |
| `k_form` | 3.597157e-07 | **3.748887e-07（導出）** | |

**五個自由參數沒有一個貼界** —— 連 HEAD 一直頂在上界 100 的 `K_P_sost` 都落到 91.4。這比 HEAD 的校正**更適定**。

### C35.4 標的：一失、兩得、盲測全存活

| | HEAD | P5o | |
|---|---|---|---|
| V1 | 7.034 ✅ | 7.331 ✅ | |
| V2 | 1.185 ✅ | 1.220 ✅ | |
| **V7** | 0.716 ✅ | **0.663 ❌** | **唯一的損失**（差 0.037，容差 0.8） |
| V7 平台 | −0.140 ✅ | −0.131 ✅ | |
| V8 | 12.042 ✅ | 12.459 ✅ | |
| V16 | 1.183 ❌ | 1.124 ❌ | 本來就未達 |
| **V10**（盲測） | −11.38 | **−8.70** | **改善**：24 個月收在基線**上** 2.68 %（原為下 0.75 %） |
| V14（盲測） | 786.77 ✅ | 786.77 ✅ | 不變 |
| **V6a–f**（盲測） | 6/6 ✅ | **6/6 ✅** | 全數存活 |
| P2 | 成立 | 成立 | |
| P3 | 單穩 | **單穩**（回復 100.3 %） | |
| **V11 十年回復** | 63.5 % | **146.5 %** | 由部分變完整 |
| chi² | 3.44 | 4.81 | |

**V7 是唯一的損失，而它的方向是有訊息量的**：鈣這條臂原本被一個「十天停留時間」造成的骨細胞密度回饋放大。移除假影使鈣**更沒有效** —— 與 C27.7 對 P5k 的解讀同型，**強化 P1 的許可性那一半**。

依 V16 的既有慣例處理（宣告容差斷言、`pass` 保持 false、稿件如實報告、留反向守門）—— 見 `testCalciumEffect_V7`。

### C35.5 可辨識性：`delta_ab` 由可辨識變成不可辨識

重跑（`experiments/exportS1FigIdentifiability.m`，**新建 —— S1 Fig 原本沒有腳本，無法重製，與「標的訂了沒人跑」同一類缺陷**）：

| | HEAD | P5o |
|---|---|---|
| (K_P_sost, lambda_P) | 不可辨識，68.6 %、r = −0.28 | 不可辨識，**76.9 %**、r = **−0.26** |
| `k_res` | 可辨識 | 可辨識 |
| `K_S` | 可辨識 | 可辨識 |
| **`delta_ab`** | 可辨識 | **不可辨識** |

`delta_ab` 的條件 χ² **單調下降到上界** —— 因為中點度量把 V8 往帶中心 13.3 拉，而它搆不到。**報告的值滿足該帶，但不是點估計**，稿件已如此陳述。

### C35.6 C33 在修正後的模型下**反轉**

C33 的全部數字都是在壞掉的 `k_ot` 下量的，必須重量。重量後結論變了：

| `lambda_apop`（s2 = 0.5） | 0 | 3 | 6 | 10（上界） |
|---|---|---|---|---|
| **V2** | 0.440 | 0.457 | 0.474 | **0.496** |

**推到上界也只到 0.496，離 1.0–1.5 帶很遠。** C33 說「補得上但買不起」；**現在是「根本補不上」** —— 因為驅動它的骨細胞缺口由 20.7 % 變成 **2.24 %**，而**那 20.7 % 本身就是 514 倍假象的產物**（C34.6 已預告）。

> **這一條是本輪最重要的方法論**：C33 的第一次量測會是一個站得住腳的已發表結果，**而它錯得沒有任何標的抓得到**。稿件現在兩次量測都報告。

（附帶：`s2_Sv = 0.5` 在修正後反而讓 **V7 = 0.724 進帶**、`lambda_apop = 10` 讓 **V16 = 1.354 進帶** —— 但 V2 塌掉。取捨換了位置，沒有消失。）

不可逆性發現存續且更強：s2 = 0.5 十年後 **−5.74 %、回復 0 %**；s2 = 3 則 **+3.40 %、回復 146.5 %**。

### C35.7 三份稿件的完整數值對帳 —— 三個檔案停在三個不同的參數世代

對帳做完了，而發現比預期嚴重：**同一個量在三份檔案裡有三個值**。

| | `.md` | `.tex` | `zh-TW.md` |
|---|---|---|---|
| romosozumab @12 mo | +12.459 | **+12.461** | **+12.179** |
| 休息插入增益（中／高幅值） | 1.62／1.43 | **1.64／1.44** | 1.62／1.43 |
| 缺鈣 vs 補充 | +3.994／+4.756 | **+3.938／+4.878** | +3.994／+4.756 |

這與 v2.13 查到的**引註**分歧同型（`.md` 3 處、`.tex` 18 處，而投稿用的 `.docx` 由 `.md` 產生，正是最少的那份），只是這次是**數字**。

### C35.8 更嚴重的是：兩份都對，也仍然是錯的

對帳過程中發現**三份檔案都比模型舊**，其中兩項是實質結果：

**一、休息插入（V5）的吻合已經消失。** 稿件宣稱「實得 6.02 倍對實測 5.8 倍，吻合至 4 % 以內」—— 這是 §0b 三列為「副產品」的亮點。實測舊參數確實給 6.02，**但 P5o 之後是 4.60**：

| | ×1.5 | ×3.0 | ×4.5 |
|---|---|---|---|
| P5o 前 | **6.02** | 1.62 | 1.43 |
| P5o 後 | **4.60** | 1.66 | 1.45 |

**對 Srinivasan 的 5.8 由「吻合 4 % 內」變成「短少 21 %」。** 這個量的歷史是：Currey 指數之前 13.99 → 之後 6.02 → `k_ot` 修正後 4.60。**三次修正沒有一次是衝著它來的，卻把它推過實測值再推出另一邊。** 標的真正要求的**條件性形狀**（低幅值大增益、隨幅值消退）三次都存活。

> 這一條要寫進稿件，因為它是對「把接近的吻合當成驗證」最好的反例 —— 而且反例是我們自己。

**二、P3 的零斜線描述已不成立。** 稿件寫「不動點由 0.985 滑到 0.097，在 E₂ 0.93–0.90 之間陡峭但連續地通過」。P5o 之後 E6 給的是 **0.078 → 0.059，平緩、無陡過渡**，而且**每一個 E₂ 下都落在線性彈性域地板（f_bm = 0.468）之下**。

**結論（單穩、無鞍結、無遲滯）沒有變，描述變了。** `test_bifurcation` 通過、E6 印「monostable: 1」把它整個蓋住了 —— **判準因為對的理由通過，周邊敘述卻已作廢**。稿件已改寫，並加上該構造落在有效域之外的但書，把重量移到全模型遲滯探針（全程 `valid = 1`，回復 100.3 %）。

### C35.9 處置

三份已全部對到**當前模型輸出**（不是互相對齊 —— C35.8 證明互相對齊不夠）。新增 `tools/reconcile_manuscript_numbers.py`：逐段對齊後比對段內數字，所以輸出會指名是哪一句。殘餘差異只剩標記假影（指數寫法、χ² 上標、ORCID 行）。

> **給下一輪的規則**：任何改動模型參數之後，**先跑 E0–E7，再跑這支對帳工具**，最後才重建 docx。這一輪是反過來做的（先改稿件、後對帳），所以才會在對完帳之後又發現兩個實質結果變了。

---

## 附錄 C36：P5p —— `lambda_xi` 的力學版。這一次機制是對的，敗在生理（v2.25, 2026-07-31）

### C36.1 先做檢查，這次是在動手之前

C33.8 把 `lambda_xi`（表面分配偏移）列為廢用缺口的最後候選，並要求先確認它有沒有 C33.4 的同型病灶。**有，而且更徹底。**

`lambda_xi` 的驅動量是 TNF-α，而 TNF-α 的唯一輸入是雌激素。實測 T 在各情境的終值：

| sedentary | bedrest | lowCalcium | romosozumab | 停經 |
|---|---|---|---|---|
| 1.000000 | **1.000000** | 1.000000 | 1.000000 | 4.807692 |

**在廢用時精確不動。** 不是「不夠情境專屬」，是**完全不作用** —— 它只在停經情境下開。C33.8 的候選如規格所寫是死的。

### C36.2 力學版：而這一次，先量專一性再寫程式

`lambda_apop` 與 `lambda_ot_mech` 都是**建完才發現驅動量不專一**。這次反過來，先量劑量缺口 `max(0, 1 − D̂)`：

| bedrest | romosozumab | lowCalcium | highCalcium | sedentary | resistance |
|---|---|---|---|---|---|
| **0.886** | 0.018 | **0.0016** | 0.000 | 0.0007 | 0.000 |

**對廢用的專一性是低鈣臂的 550 倍** —— 遠優於 `gA`（缺口比 80:1，還被 2.5 年複利吃掉）。**這一項值得建。**

方向有人體佐證：**Rolvien et al. 2020**（doi:10.1002/jbmr.3970）報告長期臥床者股骨皮質**孔隙度顯著較高**，且型態與停經後骨鬆可區分 —— 廢用往**皮質內**吸收。

實作：`xi_i ~ xi_i_0 · (1 + lambda_xi_mech · max(0, 1 − D̂))`，與另兩項同構（寫在缺口上，基線恆為 1）。與 `lambda_xi` 是**不同表面、不同驅動量的兩項**，不是一項兩因。

### C36.3 它可辨識，而且和 `K_S` 不簡併

依 C35.2 立下的規矩，先量槓桿再信任自由清單。

**`K_S` 單獨掃到下界只到 V2 = 0.9866**（重現 C30.5 的「掃到界仍達不到」）。而在相近 V2 下兩條路的**幾何指紋明顯不同**：

| | V2 | 廢用 180 d 的 r_e | V8 |
|---|---|---|---|
| `K_S` 路線 | 0.957 | **+0.0378 mm**（骨內膜擴張） | 13.79 |
| `xi_mech` 路線 | 0.849 | **−0.0019 mm**（幾乎不動） | 12.57 |

**不是簡併** —— 這與 `sost_reb` 的情況不同（C35.2）。

### C36.4 它確實補得上 V2，而且 V7 全程在帶內

`s2_Sv = 0.5`、其餘維持 P5o 值：

| `lambda_xi_mech` | 0 | 2 | 4 | 8 | 12 | 20 |
|---|---|---|---|---|---|---|
| **V2** | 0.440 | 0.733 | 0.849 | 0.949 | 0.994 | **1.036** |
| **V7** | 0.724 ✅ | 0.737 ✅ | 0.749 ✅ | 0.771 ✅ | 0.791 ✅ | 0.823 ✅ |
| V8 | 12.46 ✅ | 12.57 ✅ | 12.57 ✅ | 12.45 ✅ | 12.45 ✅ | 12.13 ✅ |
| V10 | −8.70 | −9.69 | −10.52 | −11.87 | −12.92 | −14.59 |
| chi² | 15.07 | 8.74 | 7.02 | 5.96 | 5.53 | 5.51 |

**而且注意 V7**：`s2_Sv = 0.5` 在 P5o 基線下讓 V7 = 0.724 **通過** —— 採用實測指數會把 P5o 損失的那個標的**贏回來**。

六參數重擬（把 `lambda_xi_mech` 放進自由清單）給 fval **0.0642**（出貨值 0.6017）：V2 = 1.011 ✅、V7 = 1.038 ✅、V8 = 14.47 ✅。

### C36.5 但它敗在生理，不是敗在槓桿

**`lambda_xi_mech` 要到 12–18 才夠。那時廢用的表面分配是：**

| `lambda_xi_mech` | 骨膜 | 骨內膜 | 皮質內 |
|---|---|---|---|
| 0（基線） | 0.050 | **0.450** | 0.500 |
| 12 | 0.008 | **0.071** | 0.921 |
| **18.21（擬合值）** | 0.006 | **0.050** | **0.945** |

**廢用時 94.5 % 的吸收變成皮質內，骨內膜由 45 % 掉到 5.0 %。** 人體臥床骨流失有相當部分是骨內膜的（骨內膜小樑化）。把它壓到 5 % **不是機制，是旋鈕**。

六參數重擬另外還付了 **V1 = 10.2167（帶 5–10，未達）**，`k_res` 比出貨值高 39 % —— 而 V1 的 5–10 帶本身已是 Parfitt 3–10 的上半部（§0b.四），10.22 高於任何被引用的值。

### C36.6 裁決與模式

**不採用。** `s2_Sv` 維持 3，`lambda_xi_mech = 0` 出貨，移出自由清單，由 `test_apoptoticArm.m` 守住（91 測試）。

**三個廢用機制，三種不同的失敗方式** —— 這個模式本身才是結果：

| | 專一性 | 權威 | 生理 | 敗在 |
|---|---|---|---|---|
| `lambda_apop`（C32–33） | ❌ 由吸收驅動 | ✅ | — | 不專一 → 毀掉 P1 平台；P5o 後連權威也沒了 |
| `lambda_ot_mech`（C33.5） | ✅ | ❌ 凋亡只佔骨細胞移除 1 % | — | 沒有權威 |
| **`lambda_xi_mech`（C36）** | ✅ **550:1** | ✅ | ❌ **94.5 % 皮質內** | **生理** |

> **三者依序排除了三個不同的環節**，而每一個的失敗理由都不是前一個的理由。這已經強到可以當成一項結構性陳述：**在本模型裡，以實測比表面積指數重現廢用流失，需要一個目前三條路都給不出的機制。**

### C36.7 一個新的、值得單獨記的發現

**C30.5 說「V2 在實測指數下無法達成」，那是把 `K_S` 當唯一旋鈕量出來的。放開 `k_res` 之後 V2 是達得到的**（六參數重擬給 1.011），只是要付 V1。

所以正確的陳述不是「達不到」，而是：**每一條已知路徑都用另一個標的來付帳** —— V1、V2 或鈣的平台。這是一個約束，不是調參失敗。

### C36.8 ⚠️ 踩雷點：`getDefaultParams` 的 persistent cache 讓一次 18 分鐘的校正跑錯參數

`calibrate` 呼叫的是 `getDefaultParams()`（**沒有** `reload`）。改完 CSV 直接跑 calibrate，拿到的是**這個 session 先前快取的舊值**。

本輪第一次 P5p 校正就是這樣：改了 `s2_Sv = 0.5`，跑了 18 分鐘，**實際跑的是 s2_Sv = 3**，而結果看起來完全合理（`lambda_xi_mech` → 0，因為在 s2 = 3 下本來就不需要它）。是印出 `pF.s2_Sv` 才發現的。

> **規則**：任何腳本在 `calibrate` / `probeTargets` / `identifiability` 之前，**必須先 `getDefaultParams(reload = true)` 並把關鍵值印出來**。修改 CSV 後不重載，失敗是**無聲且看起來合理**的。

---

---

*文件結束｜v2.25（C36：P5p —— `lambda_xi` 在廢用時精確不動；力學版專一性 550:1、可辨識、補得上 V2，但需要 94.5 % 皮質內分配，敗在生理；三個廢用機制三種失敗方式）｜C35：P5o —— `k_ot` 改為導出、五參數重擬、`sost_reb` 移出自由清單、C33 反轉為「根本補不上」；C35.7–9 三份稿件完整數值對帳，查出 V5 吻合消失與 P3 零斜線描述作廢）｜C34（`k_ot` 快 500 倍 —— 模型自相矛盾 514 倍，Buenzli & Sims 獨立確認；`delta_ot_0` 的比例問題本身問錯了；`lambda_ot_mech` 倍率查得 0.66 對 C33 的 200；Plotkin 2015 以實驗獨立複製 C33 的結論）｜C33：P5n′ 執行 —— n_ot→RANKL 補得上 V2 但買不起 P1 平台，力學凋亡不救、實測指數使流失不可逆，兩機制以 0 出貨）｜C32：n_ot→RANKL 實作規格；C31：P5n′ 起步，排除感測增益一路；C30：Lerebours 證實 S_v 同一病灶；C29：P5m 小樑覆寫 kappa_E；C28：P5l 採用 nu_E=2.55、否決 kappa_E=3.13；C27：P5k 鈣模組兩半重建、重擬、V7/V16 結構衝突｜2026-07-30*
