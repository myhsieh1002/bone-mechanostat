...
function testUnloadingSurfaceBiasIsNeutralAtBaselineAndOneSided(tc)
% The third disuse mechanism (P5p, appendix C36): unloading shifts
% resorption to the intracortical surface, which is what raises cortical
% porosity in immobilisation.  Same construction as the other two -- written
% on the dose DEFICIT, so exactly inert at the reference state.
%
% This one failed on physiology rather than on leverage.  It is both
% disuse-specific (dose deficit 0.886 in bed rest against 0.0016 in the
% low-calcium arm) and strong enough to close the V2 gap, but the split it
% needs to do so is 94.5 % intracortical against 5.0 % endocortical, and
% human bed rest resorbs endocortically too.  See C36.
p = tc.TestData.p;
mech = struct(eps_p = 8e-4, eps_e = 6e-4, eps_bar = 7e-4);
dose = @(e) e * 1e3;                       % monotone; the shape is irrelevant here

[~, xiBase] = surfaceAllocation(mech, 1, 1.0, dose, p);
for lam = [0 5 20]
    q = p;  q.lambda_xi_mech = lam;
    [~, xi] = surfaceAllocation(mech, 1, 1.0, dose, q);
    verifyEqual(tc, xi, xiBase, ...
        "The resorption split at the reference dose moved with " + ...
        "lambda_xi_mech = " + lam + "; the unloading bias is not written " + ...
        "on the dose deficit.", AbsTol = 1e-14);
end

q = p;  q.lambda_xi_mech = 5;
[~, xiLo] = surfaceAllocation(mech, 1, 0.2, dose, q);   % unloaded
[~, xiHi] = surfaceAllocation(mech, 1, 2.0, dose, q);   % over-loaded
verifyGreaterThan(tc, xiLo(3), xiBase(3), ...
    "Unloading must shift resorption towards the intracortical surface.");
verifyEqual(tc, xiHi, xiBase, ...
    "The bias is one-sided: over-loading must not shift resorption at all.", ...
    AbsTol = 1e-14);
end
...
verifyEqual(tc, p.lambda_xi_mech, 0, ...
    "lambda_xi_mech is no longer zero.  Read appendix C36 before proceeding: " + ...
    "this one works, and it is shipped off because the surface split it " + ...
    "needs is 94.5 % intracortical.  If you are enabling it, say in the " + ...
    "manuscript what the endocortical share becomes.");

% The exponent these three mechanisms exist to make adoptable.
verifyEqual(tc, p.s2_Sv, 3.0, ...
    "s2_Sv has moved off the shipped value.  The measured cortical value is " + ...
    "0.5 (Lerebours 2015) and adopting it is the goal of appendices C30-C36; " + ...
    "if you have just adopted it, check which target paid -- every route " + ...
    "found so far costs V1, V2 or the calcium plateau.", AbsTol = 1e-12);
end
