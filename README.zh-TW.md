...
> **P1 三個子句都帶度量限定**（C23）：子句 1 以 aBMD、子句 2 以 **BMC** 且負荷須達 ≈2900 με、子句 3 以**絕對框架**。
> 換度量會翻轉子句 2 與 3 的結論 —— 這本身是本模型的一項方法學結果。

MATLAB R2026a ｜ 計畫書：[PROJECT_PLAN_bone_mechanostat.md](PROJECT_PLAN_bone_mechanostat.md)（v2.25，附錄 C1–C36）

---

## ⚠️ 讀這份之前先看這裡

**本檔是中文工作紀錄，按時間累積而成，下面「P1 驗證結果」「P2 結果」「P4 全模型校正」三節是當時的歷史快照，數值已被後續輪次取代。當前值以本節為準**（對外英文版 [README.md](README.md) 已同步為當前值）。

**v2.25｜91 測試 0 失敗｜已於 2026-08-04 投稿 PLOS Computational Biology**（編號 `PCOMPBIOL-D-26-01926`）。
Zenodo：concept DOI `10.5281/zenodo.21592303`｜v2.25 版本 DOI `10.5281/zenodo.21784609`。

當前標的值（2026-08-04 實測，`evalTargets(getDefaultParams(reload=true), holdout=true)`）：

| 標的 | 值 | 帶 | |
|---|---|---|---|
| V1 骨轉換率 | 7.3306 %/yr | 5–10 | ✅ |
| V2 廢用流失 | 1.2202 %/mo | 1.0–1.5 | ✅ |
| V7 鈣效應 | **0.6630 %** | 0.7–1.8 | ❌ **未達**，如實出貨 |
| V7 平台斜率 | −0.1307 %/yr | \|·\|<0.3 | ✅ |
| V8 romosozumab@12mo | 12.4593 % | 11–14 | ✅ |
| V16 停藥後 CTX 過衝 | **1.1238** | 1.2–1.4 | ❌ **未達**，帶為自訂 |
| V10 停藥回落（盲測） | −8.6991 % | <0 | ✅ |
| V14 湧現 ε\*（盲測） | 786.7704 με | 100–1500 | ✅ |

自由參數**五個**（`k_res`, `K_S`, `K_P_sost`, `lambda_P`, `delta_ab`）擬合**五個**標的；
`k_form`（骨量平衡）與 `k_ot`（骨轉換）於每次評估**導出而非擬合**。八項盲測全過。
...
| **P7** | 論文 | ✅ **已投稿**（2026-08-04，PLOS Comp Biol `PCOMPBIOL-D-26-01926`）：P1✅／P2✅／**P3❌ 如實入主論文** |

**M1–M8 全模型可跑並已校正，含雙腔室、小樑腔室與分歧分析；91 測試 0 失敗。** `rhsFull` 串起完整訊號鏈：
...
## P1 驗證結果（MATLAB R2026a，2026-07-24）〔歷史快照〕
...
## P4 全模型校正（2026-07-25）— 里程碑〔歷史快照，數值已被取代〕
...
| V8 romosozumab@12mo | +12.8 % | 11–14 | ⚠️ 見下（**當前值 12.4593 ✅**，經 C20 小樑腔室重擬與 C35 的 k_ot 修正） |
| **V10 停藥回落**（盲測） | **−5.8 %** | <0 | ⚠️ 見下（**當前值 −8.6991 ✅**；C21 反彈機制先推到 −12.4，C35 導出 k_ot 後回到 −8.70） |
| **V14 湧現 ε\***（盲測） | **762 με** | 100–1500 | ✅（**當前值 786.7704**） |
...
- [x] ⚠️ **Fu 2025 已到手**（v2.18）**但解不開通道速率**：正文給的是三態 ODE 的**六個**速率 k1–k6（兩個是剪應力的函數），與我們的四個不是一對一，數值落在館際掃描不含的 Appendix。仍需 Fu 的補充材料，或其動力學來源 Mao 2022 Biophys J 121:4740（**手指觸覺**機電模型，非骨骼）
- [x] ✅ **Weinbaum 1994 已到手**（v2.20–21）：未直接給 `K_tau`，但把剪應力驗證帶由我們自訂的 0.8–3.0 Pa **收緊為原文的 1.0–2.0 Pa**；基線久坐 1.57 Pa 仍在帶內，`K_tau` 信心度 low → medium
- [ ] Martin 1984（#6b）→ $S_v$：**`s2_Sv` 已知比實測陡約六倍**（d ln S_v/d ln f_bm = −56，Lerebours 隱含約 −9.5）。三個補救機制全數失敗（見下），已如實寫入稿件 Limitations
...
- [x] ⚠️ **`sost_reb` 已於 P5o 移出自由參數清單**（自由參數 6→5）：實測其槓桿不足以支撐擬合身分。仍待 romosozumab 治療下的 SOST mRNA／游離硬化蛋白時序來獨立確認其值
...
- [ ] **V5b 骨層級符號仍與實驗相反**（C18.3）。Fu 2025 到手後未能複驗（速率數值不在文內，見上），這仍是佔位通道常數最尖銳的可證偽後果，已如實寫入稿件
...
- [x] ✅ **P5m 彈性律指數分腔室採用**（C31）：Currey 1988 的 3.13 只適用緻密骨，小樑由 `trabecularParams` 覆寫為 Gibson–Ashby 的 2.00。P5l 曾全域套 3.13 而必須撤回
- [x] ❌ **廢用機制三連敗**（C33–C36）：`lambda_apop`（不具情境專一性 → 毀掉 P1 平台）、`lambda_ot_mech`（有專一性但無權重 —— 凋亡僅佔骨細胞移除的 1%）、`lambda_xi_mech`（專一性 550:1 且有權重，但需要 94.5% 皮質內再吸收 —— 敗在生理）。**三者皆以 0 出貨**，各自敗在不同環節，已全部寫入稿件
- [x] ✅ **P5o 修掉 514× 內部矛盾**（C35）：`k_ot` 改由骨轉換率導出（`osteocyteBurialRate.m`），比值精確為 1.00000。代價是 V7 由 0.716 掉到 0.663（未達），收穫是 V10 由 −11.38 改善至 −8.70，且 `K_P_sost` 脫離邊界
