...
**接手者請先讀這份，再讀 `PROJECT_PLAN_bone_mechanostat.md`（v2.23，含附錄 C1–C34）與 `README.zh-TW.md`（中文工作版；`README.md` 為對外英文版）。**
...
**版本 v2.23｜90 測試 0 失敗｜稿件與 HEAD 完全一致**（三格式 + 兩份 docx + 七張 TIFF 全部是當前值）。版本庫處於**已驗證**狀態 —— 動任何東西之前先確認你能把它跑回這個狀態。
...
### ▶ `delta_ot_0` / `k_ot` 出處已查（附錄 C34）—— 查出一個 514 倍的內部矛盾

**`k_ot` 快了約 500 倍，而這一條不需要文獻就看得出來。**

骨細胞是隨著它所在的骨被吸收才離開組織的，所以骨細胞族群的分數移除率**必須等於**骨的分數周轉率：
...
**Buenzli & Sims 2015** 獨立確認是哪一邊對：420 億個骨細胞、每天補充 910 萬 → **2.167 × 10⁻⁴ /day**、停留 **12.6 年**（模型是 10 天）。文獻導出值 `k_ot = 1.083 × 10⁻³ /day`。

**`delta_ot_0` 沒找到直接出處，但問題本身問錯了。** C33.8 問「1 % 這個**比例**對不對」；實際壞掉的是**整體速率**，而比例的分子分母是同一個錯誤參數算出來的 —— 修正 `k_ot` 後把比例掃 1 %→25 %，V7 只動 0.012、V2 完全不動。

**這反過來加強 C33，並廢掉 C32.1 的前提**：廢用 180 天的骨細胞缺口在修正速率下是 **2.3 %**，不是 C32.1 所依據的 20.7 %。

### ▶ 第一件事：P5o —— 採用 `k_ot = 1.083e-3` 並重擬

**未在 v2.23 修**，因為它讓 **V7 掉到 0.6255**（帶 0.7–1.8）、**V8 掉到 10.57**（帶 11.9–14.7），需要一次重擬，而稿件目前與 HEAD 完全一致。模型本身**物理上沒問題**（漂移正常、`validity.ok = 1`、P3 解析上不受影響），**V10 反而變好**（−11.38 → −8.59）、V11 十年回復 63.5 % → 144.2 %。這是 **P5m 型**（該重擬）而非 P5l 型（該撤回）。

重擬要點：

1. **V8 的旋鈕是 `delta_ab`，情境專屬**（踩雷點 #5），所以修 V8 不會動 V7 —— 單參數重擬。
2. **V7 沒有專屬旋鈕**，P5k 之後它就坐在區間下緣。差 0.0745。**這是 P5o 真正的難點。**
3. 複驗三組盲測（V6a–f、V10、V14）與 P3 三項；圖表與稿件數字會變。
4. 已用 `testOsteocyteRemovalIsInconsistentWithBoneTurnoverByKnownFactor` 把 514 這個比值釘住 —— **那個測試預期會在修好的那天失敗**，訊息裡寫明那正是預期的修法，改的時候連 C34 一起更新，不要刪測試。

其次是 C33.8 第 1 項的 `lambda_xi`（表面分配偏移）。**動手前先確認它有沒有 C33.4 的同型病灶**：`lambda_xi` 由 TNF-α 驅動，而 TNF-α 目前只看雌激素、不看力學 —— 與凋亡當初的問題完全一樣。
...
**版本 v2.23｜90 測試 0 失敗｜稿件與 HEAD 完全一致（三格式 + 兩份 docx + 七張 TIFF 皆為當前值）**
...
| **P5n′** `n_ot → RANKL` + 力學凋亡 | ❌ **建成、量完、未採用**：補得上 V2（1.023），代價是 P1 的平台（C33）。兩機制以 0 值出貨，6 項新測試守住 |
...
*Handoff 更新於 2026-07-30｜v2.4/61 測試 → **v2.23/90 測試、附錄 C16–C34、稿件與 HEAD 一致**｜P5n′ 已執行完畢並判定不採用（C33）；`k_ot` 出處已查出 514 倍內部矛盾（C34）；下一步見 §0：P5o 採用 `k_ot = 1.083e-3` 並重擬*
