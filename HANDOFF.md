...
**接手者請先讀這份，再讀 `PROJECT_PLAN_bone_mechanostat.md`（v2.13，含附錄 C1–C25）與 `README.zh-TW.md`（中文工作版；`README.md` 為對外英文版）。**
...
已用同一套標準逐節檢查並修正 **P1、P2、P3、V4、V5、V5b、V9、V12**，以及 **V7b、V11、V13、V15**（見附錄 C24 —— 這四個先前**從未被任何實驗執行過**，v2.13 新增 `experiments/E7_qualitativeTargets.m` 跑完，結果**一過三敗**）。

**V8 / V10 / V16 亦已完成**（附錄 C25）。當中查出稿件的 **V10 數字是錯的**（−13.967 → 實際 **−12.416**），並補上唯一直接支持停藥主張的文獻（McClung 2018, PMID 29694685，現為文獻 23 —— 先前 V10 與 V16 兩項主張**完全沒有引註**）。

### 尚未檢查

**V1、V2、V3、V7**

四者都是校正標的（非盲測），風險較低，但仍應照同一把尺過一次：

- **V3** 的 48.8 %（廢用時硬化蛋白上升）**沒有判準也沒有引註**。標的本身只要求「方向為上升」，稿件卻直接給了一個數字，讀者無從判斷那算不算通過。
- **V1 / V2 / V7** 在稿件中都寫了判準帶且數字對得上，主要要確認的是：數字是否仍能由當前 HEAD 重跑得到（見下一則教訓）。

### ⚠️ 送出前務必做一次：全數字對帳

C25.1 的教訓 —— 稿件寫 −13.967 %，實際是 −12.416 %，那是**舊參數留下的殘值**。同批還查出 V8cort、負荷單獨、併用、次相加四個數字微漂。

```matlab
r = evalTargets(getDefaultParams(reload=true), holdout=true);   % 逐項對照 Results
```
...
**已知缺陷（v2.13，C24）**：血清鈣隨飲食擺動 15 %（生理約 ±2 %）；V13 的 RANKL 反應有結構性天花板 1.33 倍；模型不產生停經後骨膜外擴。三項均已如實寫入稿件 Limitations。

---

## 2. 現況：v2.13，81 測試全過，已公開並取得 DOI
...
*Handoff 更新於 2026-07-26｜v2.4/61 測試（Opus 4.8）→ **v2.13/81 測試、附錄 C16–C24、投稿就緒**（Opus 5）｜未竟事項見 §0*
