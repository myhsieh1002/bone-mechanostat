...
**接手者請先讀這份，再讀 `PROJECT_PLAN_bone_mechanostat.md`（v2.22，含附錄 C1–C32）與 `README.zh-TW.md`（中文工作版；`README.md` 為對外英文版）。**
...
**版本 v2.22｜84 測試 0 失敗｜稿件與 HEAD 完全一致**（三格式 + 兩份 docx + 七張 TIFF 全部是當前值）。版本庫處於**已驗證**狀態 —— 動任何東西之前先確認你能把它跑回這個狀態。

```matlab
cd('.../骨骼鈣質吸收數學模型'); addpath('src'); setupPath(); maxNumCompThreads(4);
runtests("tests")                                    % 應為 84 passed / 0 failed
evalTargets(getDefaultParams(reload=true), holdout=true)
```

### ▶ 第一件事：執行 P5n′（附錄 C32）

診斷已經做完，**C32 是可以照著做的完整規格**：`n_ot → RANKL` 新增項、四處程式改動（呼叫端已 grep 確認只有兩處）、單參數校正程序、六項驗收清單、以及「若失敗該怎麼記錄」。

**請一次做完。** 這是會動到校正的結構改動；做一半（簽章改了、未校正、測試紅的）比不做更糟。

背景：C31.4 實測 SOST 力學臂在廢用時飽和到 **99.97 %**，把 YAP/TAZ 壓到零也買不到額外倍率 —— 所以**任何經由感測的機制都無效**，包括力學驅動的凋亡。缺口只能由繞過硬化蛋白的路徑供給，這就是 C32 的由來。
...
**版本 v2.22｜84 測試 0 失敗｜稿件與 HEAD 完全一致（三格式 + 兩份 docx + 七張 TIFF 皆為當前值）**
...
>
> **待辦 P5n′ —— 診斷已完成，規格已寫好，只差執行**：
>
> - C31.4 實測：SOST 力學臂在廢用時飽和到 **99.97 %**，把 YAP/TAZ 壓到零也買不到任何額外倍率 → **任何經由感測的機制都無效**，包括力學驅動的凋亡。
> - 缺口只能由**繞過硬化蛋白**的路徑供給。
> - **`附錄 C32` 是可照著做的完整規格**：`n_ot → RANKL` 新增項、四處程式改動（已逐一 grep 確認）、單參數校正程序、六項驗收清單。
>
> **請一次做完。** 這是會動到校正的結構改動，做一半（簽章改了、未校正、測試紅的）比不做更糟 —— 版本庫現在是 84 測試 0 失敗的已驗證狀態。
...
*Handoff 更新於 2026-07-28｜v2.4/61 測試 → **v2.22/84 測試、附錄 C16–C32、稿件與 HEAD 一致**｜下一步見 §0：執行 P5n′（附錄 C32）*
