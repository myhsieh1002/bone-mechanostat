...
**原本暫緩等 Fu，該理由已失效**（2026-07-28）。Fu 2025 到手後發現它只有**方程式**、沒有**數值**（數值在館際掃描不含的 Appendix；且原始出處是 Mao 2022 的手指觸覺模型）。四個常數仍是佔位值，**V5b 不會因為 Fu 而改變**，所以沒有再等的理由。

決定：**直接把剩餘對帳與 TIFF 做完**，V5b 照現行寫法誠實標為「架在佔位常數上」—— 那個寫法本來就站得住。
