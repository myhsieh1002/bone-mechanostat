...
**接手者請先讀這份，再讀 `PROJECT_PLAN_bone_mechanostat.md`（v2.13，含附錄 C1–C26）與 `README.zh-TW.md`（中文工作版；`README.md` 為對外英文版）。**
...
| **模型缺陷 1** | 血清鈣隨飲食擺動 **15 %**（生理約 ±2 %）—— C24.5 |
| **模型缺陷 2** | **骨→血的耦合數值上可忽略**（廢用時 PTH 只動 −0.01 %，臨床 −17~−24 %）—— C26.5 |

兩個缺陷都在鈣模組，合併為待辦 **P5k**（C26.6）。**兩者都是結構性改動，會動到校正，不是投稿前該做的事** —— 現階段的正確處置是如實揭露，已完成。
