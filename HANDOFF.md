...
版本推進 v2.4 → **v2.11**，測試 61 → **81**，新增附錄 **C16–C23**。各項定論：
...
| **P5h** | 停藥反彈機制（C21） | 新增狀態 **`A_reb`**（抗體暴露下 SOST 轉錄代償上調，16→17 態）。**V10 +1.683% → −12.416%，達標且維持 hold-out 身分**（校正到新標的 **V16 停藥後 CTX 過衝**，非 BMD）。**V9 強形式一併解決**；`delta_ab` 再擬為 1.65048 |
| **P5i** | modeling drift（C22） | 「吸收改為形成的倒數」**實測方向相反**（通道劑量飽和），已回退。改走 **Frost drift 成對性**：`dr_e += chi_drift*modeling`，`chi_drift=1`（純平移，原理值）。**V6e −2.88% → +13.44%，V6 六項全過** |
| **C23** | **P1 第二句重述**（主持人裁決） | `chi_drift=1` 使子句 2 在 aBMD 上失守。主持人選：**改以 BMC 陳述 + 「適當負重」定義為 ≈2900 με**（新情境 `resistanceVigorous`）→ **+4.589% ✅**。同一對比 aBMD 僅 +1.865%（**DXA 稀釋 59%**） |

**現況：V1–V16 中僅 V5b（骨層級符號，待 Fu 2025）未達。**

⚠️ **接手時務必知道的一條敘事紀律（C23）**：**P1 三個子句各自帶度量限定** —— 子句 1 以 aBMD、子句 2 以 **BMC 且負荷須達 ≈2900 με**（`resistanceVigorous`）、子句 3 以**絕對框架**。換度量會翻轉子句 2 與 3 的結論。`E2` 的 `pass.loadInstruct`（新）與 `pass.loadInstruct_aBMD`（舊寫法對照）並存，不要混用。

**下一步候選**（無單一明顯最高價值項，請與使用者確認）：**P7 論文撰寫**（三大預測與 V1–V16 均已定論，敘事紀律見 C16/C18.2/C23）｜抗體 PK（現為階躍，C21.6）｜Sobol/LHS 全域靈敏度｜P5f bedrest 長期崩潰｜取得 Fu 2025 複驗 V5b。

<details><summary>P5h 完成前的診斷紀錄（已解決，保留供追溯）</summary> 已診斷清楚（C20.4）：這是**時間尺度**問題不是結構問題。模型的增益**確實會逆轉**，但停藥後 2.85 年才達峰、第 6 年才跌回 12 個月水準，比臨床慢約 5 倍。`lambda_S` 與 `K_L` 掃遍文獻全域都無效（`gS` 飽和使 `lambda_S` 在 `gS(S)/gS(1)` 中相消；且硬化蛋白過衝本身只有 +7.2%，那是 mechanostat 的後果不是參數）。缺的是**藥理層面的停藥反彈**（臨床 CTX 會衝到基線以上）。
...
4. **`evalTargets` 現在會呼叫 `trabecularParams`**（V8/V10/V16 算在小樑腔室）。故 `trabecularParams` **絕不可**再呼叫 `evalTargets`，否則無限遞迴 —— 它改用閉式的 `turnoverRate.m`。
5. **`localDrugOn` 用 `t < stopDay`**，故在 `t == stopDay` 當天藥已關閉。要取「治療中」的值請採樣早於 stopDay（踩過）。
6. **MCP 改完 .m 檔後 MATLAB 可能仍跑舊版**。若測試失敗訊息出現已被改掉的測試名，先 `clear functions; rehash` 再跑。
...
*Handoff 撰於 2026-07-26｜v2.4/61 測試（Opus 4.8）→ **v2.11/81 測試、附錄 C16–C23**（Opus 5，同日 A/B/D/C 四項 + P5g + P5h + P5i + C23 裁決）*
