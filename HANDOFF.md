...
### ▶ 第一件事：`manuscript.tex` 與 `manuscript.md` 的數值對帳（C35.7）

對帳稿件時發現 **`.tex` 在本輪之前就與 `.md` 分歧**（`.tex` 的 romosozumab 是 `+12.461`，`.md` 是 `+12.042`；缺鈣 `+3.938` 對 `+3.974`）。與 v2.13 的引註分歧同型，只是這次是數字。

本輪已移植七段改寫使**內容**一致，但**殘餘數值分歧未逐一清查**。`.docx`（投稿用）由 `.md` 產生，投稿路徑不受影響。**請做一次完整對帳**，並考慮改為由 `.md` 生成 `.tex`，讓這類分歧結構上不可能發生。

其次是 C33.8 第 1 項的 `lambda_xi`（表面分配偏移）。**動手前先確認它有沒有 C33.4 的同型病灶**：`lambda_xi` 由 TNF-α 驅動，而 TNF-α 只看雌激素、不看力學。
