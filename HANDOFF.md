...
### 🔴 立即待辦：稿件數字尚未與 v2.14 重擬對帳

P5k 動了校正，**稿件（三個語言版本 + 兩份 docx + 七張 TIFF）的數字全部是 v2.13 的**。下表是完整對照，逐項改即可。
...
1. **P1 子句 3 不再反轉。** 差異中的差異由 −0.536 變成 **+0.333**，也就是「鈣缺乏截斷負荷所能建造的量」在這個框架下**也成立**。稿件的招牌句「**三個子句有兩個**會在不同度量下符號反轉」**已經錯了 —— 現在只有子句 2（BMC vs aBMD）會反轉**。需改：摘要、Results P1 末段、Discussion「Measurement choice is a result」、Conclusion。
2. **V16 由達標變成未達標**，且原因是 C27.5 的結構衝突。稿件現在把 V16 寫成「落在 1.2–1.4 內」，必須改為未達並說明衝突。
