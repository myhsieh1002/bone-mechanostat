...
> ## ✅ 已於 2026-08-04 送出｜稿件編號 `PCOMPBIOL-D-26-01926`
>
> A–E 節已全部完成，保留為紀錄。**現在有用的是 F 節** —— 那是回覆審稿人時的起點。
> 送出的內容與 Zenodo DOI 見 `HANDOFF.md` 第 0 節的表格。

狀態日期 2026-07-31（送出日 2026-08-04）｜模型 v2.25｜91 測試 0 失敗
...
| ❓ | SI 檔案格式 | PLOS 對 SI 可接受的格式與命名有規定。S1 Text 已備 `.docx`；`.md` 內的 LaTeX 數學式在 Word 中是**原始碼文字**，若要排版好看需另行處理 —— 依當前規範確認 |
