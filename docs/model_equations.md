...
*The exponent is compartment-specific, and that is a third forced override.* Until v2.16 a single $\kappa_E = 2.5$ served both compartments, and the agreement it produced across two orders of magnitude of porosity was reported here as a free consistency check. It was not free: 2.5 was an assumed value, and taking the measured one exposes why it worked. Currey's power law over compact bone from 18 species gives $\kappa_E = 3.13$; applied at a bone volume fraction of 0.12 it scales the apparent modulus by $0.12^{0.63} = 0.26$, putting the vertebra at 26 MPa — below the measured 50–300 MPa range, and out of the linear-elastic domain within months. Trabecular bone is a cellular solid rather than compact bone, and Gibson–Ashby open-cell scaling gives an exponent of 2. We therefore use $\kappa_E = 3.13$ in cortex and 2.00 in the vertebra, which puts the vertebral apparent modulus at 288.0 MPa, inside the measured range. The former single value was, on this reading, correct for neither compartment: it split the difference between a compact-bone exponent and a cellular-solid one. The revised law rests on a measurement at each end rather than on a compromise in the middle — a stronger position, but no longer a claim that one exponent spans the whole porosity range.
